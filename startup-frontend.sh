#!/usr/bin/env bash
# RETIRED (v18): the Streamlit presentation service is superseded by the React
# UI in frontend-react/, which the FastAPI backend serves itself (single
# service; see app/main.py and infrastructure/azure_deploy.md). This script is
# kept so the old two-service Streamlit deployment still works if ever needed.
