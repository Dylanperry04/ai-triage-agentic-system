...
For local inspection only, the app can explicitly view the public
`mimic-iv-ed-demo-2.2.zip` sample by setting
`ALLOW_MIMIC_ED_PUBLIC_SAMPLE_VIEW=true` and `MIMIC_ED_PUBLIC_SAMPLE_ZIP`. Those
rows are labelled `MIMIC-IV-ED-Demo-v2.2`, are not training data, and the ML
prediction agent withholds predictions for them. The public-sample loader refuses
MIMIC-shaped inputs larger than the public demo subset, so a full credentialed
MIMIC zip cannot be relabelled as public demo data by mistake. There is no KTAS
dataset in the live system; earlier dual/triple-dataset designs were removed.
The only built-in demo case source is `data/demo/azure_supervisor_demo_cases.jsonl`,
a small synthetic supervisor-demo fixture clearly labelled: not real MIMIC and
not real patient data. (Historical changelog: `KTAS_CHANGELOG.md`.)
...
- **React frontend (v18)** — `frontend-react/`, built to `frontend-react/dist`
  and served BY the FastAPI backend at `/` (single service; same-origin API, no
  CORS needed for the built-in UI). Run `npm run build` in `frontend-react/`
  after UI changes; the built `dist/` is committed so Azure/Oryx needs no Node.
- **Legacy Streamlit frontend (retired)** — frontend-only; every protected action calls the
...
(safety-first comparison). The comparison uses a stratified patient-grouped split
by default; a patient-grouped shifted-date temporal split is available only as an
explicit sensitivity analysis. It selects on a validation set among candidates
passing an over-triage/specificity constraint, and reports final metrics once on
...
structured feature contract. XGBoost candidates use balanced sample weights so
the severe MIMIC class imbalance is not left entirely to the safety-first
selection criterion.
...
# Training venv: full-MIMIC model comparison and optional ML libraries
python -m venv venv-training
venv-training\Scripts\activate
pip install -r requirements.txt -r requirements-ml.txt

# Optional H100/GPU sweep tooling for the training venv only
pip install -r requirements-ml-gpu.txt
```

`requirements.txt` is the shared Azure/Oryx runtime baseline. It intentionally
includes AutoGen for the multi-agent explanation path, W&B for governance
logging, and imbalanced-learn so a selected SMOTE pipeline artefact can be
loaded by the app. Keep the heavy training-only libraries in
`requirements-ml.txt` and `requirements-ml-gpu.txt`; do not install those into
`venv-app`. Installing `requirements-autogen.txt` into `venv-training` is
redundant because those pins are already present in `requirements.txt`.
...
# Backend for public MIMIC-IV-ED demo zip viewing only (terminal 1, PowerShell)
$env:ALLOW_MIMIC_ED_PUBLIC_SAMPLE_VIEW="true"
$env:MIMIC_ED_PUBLIC_SAMPLE_ZIP="C:\Users\YOUR_USER\Downloads\mimic-iv-ed-demo-2.2.zip"
uvicorn app.main:app --host 127.0.0.1 --port 8000 --reload

# Frontend for public sample viewing (terminal 2, PowerShell)
...
Without `MIMIC_FULL_ED_DIR`, the app serves no full-MIMIC cases; the public demo
zip can be viewed only through the explicit public-sample settings above.
Without `MIMIC_FULL_MODEL_PATH`, full-MIMIC cases are visible but predictions are
withheld. If your credentialed full data is still in `mimic-iv-ed-2.2.zip`,
extract it first and point `MIMIC_FULL_ED_DIR` at the extracted `ed` folder. In
local credentialed mode, `PSEUDONYM_SECRET` and an outside-repo output/audit
directory are required so case identifiers and derived records do not silently
use the development salt or repository-local files.
