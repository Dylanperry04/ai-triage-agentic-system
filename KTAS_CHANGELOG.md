...
# Session: v10 patch — consistency/hardening + live re-run workflow + real W&B/policy governance

Verified the v10 review's claims against the actual code. ~40% was already done
in v9 (matrix fix, audit cards, tab removal, MIMIC/KTAS separation); the rest
were real and are fixed. Two items the review flagged were REAL bugs I had
introduced (#29 silent audit-error swallow, #30 duplicate audit logs on every
rerun). Two large items (#2 live re-run, #4 W&B/RAI) were confirmed in-scope by
Dylan and are now genuinely implemented (not mocked).

## Consistency / wording (all confirmed real)
- #1 top caption: no longer "KTAS cases only" — now describes dataset-specific
  estimates (MIMIC acuity + KTAS class).
- #5 removed user-facing "(ESI)" from MIMIC acuity displays (kept the code
  comment explaining KTAS-vs-ESI provenance).
- #6 Clinical Safety Assessment wording: acknowledges the provisional
  Manchester-style display rather than claiming "no category assigned".
- #7 Clinician Review now defaults to REQUEST_MORE_INFORMATION (not
  ACCEPTED_AS_PRESENTED); safer default note.

## Raw-JSON / readable displays
- #16 governance five-stage gate: readable key/value summary first, raw JSON in a
  collapsed "Developer view" expander.
- #17 MIMIC class distribution: readable table (acuity → category → max wait →
  count → %), not a raw dict.
- #15 synthetic walkthrough: readable table, raw JSON collapsed.
- #12 confirmed EVERY st.json is inside a collapsed advanced/developer/full-output
  expander (now guarded by an ast-based test).

## Audit logging hardening (two real bugs I introduced)
- #29 workflow-run audit failures now surface a non-blocking warning instead of
  being silently swallowed.
- #30 added an explicit "▶️ Run assessment" button: a workflow-run audit record
  is written only on an intentional click (one record per run), not on every
  incidental Streamlit rerun. Fits the re-run workflow.

## Dataset-aware Review Queue + case_uid (#8/#10/#11/#28)
- Review Queue now has a dataset selector (MIMIC-IV-ED Demo default, Kaggle KTAS
  separate). MIMIC queue is computed LIVE (app/agents/review_queue_builder.py):
  cases with missing fields / safety flags / fired override / unsafe-to-present
  and no saved review. Cached with @st.cache_data for performance.
- HumanReviewRecord gained a case_uid property (source_dataset:stay_id);
  get_reviews_for_case_uid added (dataset-safe). Main Triage Review review lookup
  switched to case_uid. MIMIC review saving carries source_dataset + case_uid.
  Demo reviews framed as "research-workflow review", not clinical sign-off.

## #2 — live edit-rerun-reassign workflow (Dylan's boss's mental model)
- New "✏️ Edit vitals & re-run assessment" panel in Triage Review: edit THIS
  patient's vitals, re-run the workflow, see previous → new assignment, and log a
  WorkflowRerunRecord (app/schemas/rerun.py, app/storage/rerun_repository.py) with
  changed vitals + movement (ESCALATION/DE_ESCALATION/NO_CHANGE) + reason. Proven
  end-to-end: editing a low-acuity case's HR to 195 escalates acuity 3→1 and fires
  the override. Surfaced in the Audit Log as a readable table.

## #4 — real W&B / RAI policy-as-code (not mocked)
- app/governance/policy_checks.py: EXECUTABLE policy-as-code — 4 safety invariants
  run against live cases/config (KTAS-never-gets-MTS, override-escalate-only,
  clinician-review-always-required, MIMIC-model-excludes-leakage). All PASS; can
  fail in CI. Plus policy_red_team_probes(): a small real set of adversarial
  forbidden-output probes against the actual safety filter.
- app/governance/wandb_logging.py: OPTIONAL W&B logging (wandb added to
  requirements). Logs policy/red-team/model metrics to a real W&B run
  (offline/online); no-op with a status if WANDB_API_KEY unset; never raises.
  Verified it logs 9 metrics offline.
- Governance tab: "Policy-as-code checks (executable)" + "W&B (RAI) logging"
  sections. Stale "red_team_testing: Pending" updated to "Partial" with honest
  scope (a full external red-team remains a separate effort).
- HONEST SCOPE: this is local policy-as-code + a small probe set + optional W&B
  logging, NOT a full hosted W&B Weave/eval pipeline or comprehensive red-team.

## Docs
- #34 mimic_adapter.py docstring updated (MIMIC acuity pipeline IS built; full
  MIMIC pending). Other doc stale-strings already swept in prior passes.

## Tests
- New: test_review_case_uid (3), test_rerun_workflow (3), test_governance_policy
  (4), TestReviewQueueDatasetAware (4), TestRawJsonAndReadableDisplays (4);
  obsolete KTAS-only-caption test class replaced. Full suite: 476 → 491 passing
  (429 non-frontend + 62 frontend).

## Not changed (per review "do not")
- MIMIC acuity mapping, override thresholds, dataset separation, leakage guard,
  KTAS/MIMIC prediction separation — untouched. Model remains demo-only
  (207 rows, CV ~0.52). Full MIMIC / UHL untouched.

## Genuinely Dylan's (cannot be done here)
- Run pytest in pinned Python 3.11 venv; docker build/run; Azure deploy + live
  URL; set Azure OpenAI env vars + live AutoGen; optionally set WANDB_API_KEY for
  online W&B logging.

## Build
- v10 zip built and verified on a fresh re-extract.

---

# Session: v11 — security architecture overhaul (supervisor feedback)

Supervisor feedback emphasised SECURITY for a hospital ecosystem: different user
types, secure network/VPN, confidential patient data with access requirements, ML
must produce the result (chatbot only explains), missing fields that could improve
prediction, exploring all ML techniques, and "explore whether a security layer
between agents is needed." Another LLM proposed a plan; I judged its direction
mostly right but its framing dangerously optimistic, and corrected it: most of the
asked-for controls (Entra SSO/MFA/conditional access, managed device, VPN, Key
Vault, encrypted storage) are HOSPITAL-PROVIDED infrastructure, not app code. The
app's job is to integrate with them and enforce RBAC on top, and to FAIL CLOSED
when trusted identity is absent. App RBAC alone is not security — it is the second
layer behind a real identity provider. Built in six verified phases; honest line
held throughout (public-data demo until the hospital provisions identity/network/
storage).

## Phase 1 — identity boundary + app RBAC (app/security/identity.py, authz.py,
##           access_audit.py, guard.py)
- Pluggable AuthContextProvider. AzureTrustedHeaderProvider decodes
  X-MS-CLIENT-PRINCIPAL, maps Entra groups -> internal roles, and trusts headers
  ONLY when TRUSTED_AUTH_PROXY=true. Fail-closed: PATIENT_DATA_MODE/AUTH_REQUIRED
  => no stub, real identity required. Loudly-labelled demo role-switcher for local
  public-data only (AUTH_PROVIDER=demo & not patient-data mode).
...
- Access audit logs every ALLOWED/DENIED/BLOCKED action (user/role/action/page/
  case_uid, demo-identity flag, auth source) with no raw identifiers.
- UI: sidebar identity panel + role switcher; sensitive tabs (Audit Log, Model
  Performance) gated; Run-assessment / chatbot / multi-agent gated with disabled+
  reason; demonstrated live (nurse/auditor/security_admin see different access).
- 16 tests.

## Phase 2 — Agent Security Gateway (app/security/agent_gateway.py)
- Judged that a full INTER-AGENT security layer is NOT required: the 4 agents are
  read-only explainers sharing one read-only tool; the ML decides the result
  before agents run; nothing an agent says changes it. Built the proportionate
  BOUNDARY gateway instead: authorisation (can_ask_chatbot), prompt-injection
  screening on input, evidence minimisation, deterministic output filter, and —
  the defensible version of "security between agents" — a NO-ACTION INVARIANT
  (assert_agents_have_no_action_tools) wired into the real team build that fails
  closed if any agent ever gains an action tool. Every agent call audited.
- 20 tests incl. the real-team invariant.

## Phase 3 — patient-data protection (app/security/redaction.py, secrets_provider.py,
##           audit_sink.py)
- pseudonymous_case_uid (stable, non-reversible, salted); redact_for_log drops
  identifier keys + scrubs free-text; assert_no_raw_identifiers hard guard. Access
  audit now writes THROUGH the redacting sink (guarantee on the write path).
- SecretsProvider (Key Vault seam, fails closed without a client) and AuditSink
  (encrypted-durable seam, fails closed) interfaces — config-swappable, no Azure
  dep in the sandbox; production wiring documented.
- 12 tests.

## Phase 4 — missing-data intelligence (app/ml/missing_data_intelligence.py)
- For a case: missing fields, critical missing fields, confidence-impact signal,
  and the missing field the model relies on MOST ("most useful if obtained") —
  honestly derived by aggregating the model's feature importances from the 193-dim
  one-hot space back to logical fields. Surfaced a genuine FAIRNESS finding (model
  leans on `race` ~16.5%) rather than hiding it. RBAC-gated to clinical-content
  roles. Research-signal framing. 6 tests.

## Phase 5 — model-comparison pipeline (ml_training/compare_mimic_acuity_models.py)
- Compares LogisticRegression / RandomForest / XGBoost / LightGBM / CatBoost on the
  SAME features + leakage guard. SELECTED ON TRIAGE SAFETY, NOT ACCURACY: high-
  acuity recall (primary), under-triage rate, false-negative rate, macro recall,
  calibration (Brier). Honest result: LightGBM selected (high-acuity recall 0.68,
  under-triage 0.40); XGBoost worst on recall — fancier != better on 207 rows.
  Tabular NN explicitly EXCLUDED with justification. Surfaced in Model Performance
  (RBAC-gated) with loud caveats. xgboost/lightgbm/catboost added to requirements.
  7 tests incl. the safety-metric calculations.

## Phase 6 — security architecture + full-MIMIC seam
- docs/SECURITY_ARCHITECTURE.md: the two-layer model (hospital-provided vs app-
  enforced), the role/permission matrix, config flags, the production patient-data
  profile, and honest limitations. docs/DEPLOYMENT_SECURITY_CHECKLIST.md: itemised
  App/IT checklist gating any non-public-data use.
- app/data_pipeline/mimic_full_loader.py: DUA-safe seam for full credentialed
  MIMIC. Reads from MIMIC_FULL_ED_DIR (a path the user controls), FAILS CLOSED if
  unset, REQUIRES PATIENT_DATA_MODE=true, and an ANTI-COPY guard refuses any path
  inside the repo. Credentialed data is NEVER copied into the repo/sandbox/zip.
  Sidebar shows seam-aware status. 4 tests.

## Tests / build
- Full suite 552 -> 556 across the session start; final clean re-extract count
  reported at packaging. New security/ML test files: test_security_identity_authz,
  test_agent_gateway, test_patient_data_protection, test_missing_data_intelligence,
  test_model_comparison, test_mimic_full_seam.

## Honest scope (unchanged, reinforced)
- This is a PUBLIC-DATA RESEARCH DEMO. Layer-2 controls are real and tested but are
  NOT a substitute for hospital-provided identity/network/storage and have not had
  a security review or pen test. Not clinically validated; clinician review
  required on every output. Full MIMIC / UHL data remain outside the app.

## Genuinely Dylan's (cannot be done here)
- Provision Entra/MFA/conditional access, managed devices, VPN/private ingress,
  Key Vault, encrypted durable audit store; set the production env profile; point
  MIMIC_FULL_ED_DIR at the credentialed data on your environment; run pytest in
  pinned 3.11; docker build/run; Azure deploy; independent security review.

---

# Session: v12 — deployment hardening + full-MIMIC adapter validation

Two external reviews (security gaps; full-MIMIC direction) were verified against
the code, de-duplicated, and the genuinely-needed items implemented. ~40% of the
reviews duplicated already-done work or each other; a few items were wrong/
premature (e.g. "make full MIMIC the active dataset and retrain here" — declined,
that runs only on the credentialed environment). The full credentialed MIMIC-IV-ED
zip that was uploaded was NOT extracted into the sandbox (PhysioNet DUA); only its
public README/SHA256SUMS were read for the authoritative schema.

## Full-MIMIC adapter validation (synthetic fixtures only — no credentialed data)
- Confirmed the adapter's EXPECTED_COLUMNS match the documented full schema for
  all six tables. Validated reading of the full .csv.gz format with SYNTHETIC
  fixtures (fabricated rows).
- Fixed a real bug: full cases were mislabelled source_dataset='...Demo...';
  dataframe_to_cases now takes source_dataset_label, and load_mimic_full_cases
  tags them 'MIMIC-IV-ED-Full-v2.2' (keeps dataset separation + pseudonymisation
  correct).
- Improved validate_mimic_tables to distinguish REQUIRED_MISSING (fatal) from
  TOLERABLE_DIFF (extra/reordered/absent-demographics) — so the adapter won't
  crash whichever way the full edstays schema turns out, and reports clearly.
- Added load_mimic_full_cases / validate_full_mimic_schema to the DUA-safe seam
  (still fail-closed, patient-data-mode-required, anti-copy-guarded). 6 tests.

## v12 deployment hardening (FastAPI security boundary — chosen architecture)
- app/api/auth_dependencies.py: requires(permission) FastAPI dependency reads the
  real X-MS-CLIENT-PRINCIPAL, enforces RBAC SERVER-SIDE (401 unauth, 403 wrong
  role), fails closed in patient-data mode, audits every decision. Applied to all
  14 routes across 6 route files.
- app/main.py: startup security guard refuses patient-data mode without the full
  secure config (AUTH_REQUIRED, non-demo provider, TRUSTED_AUTH_PROXY, keyvault,
  durable). Docstring updated: FastAPI is now the documented server-side boundary.
- 10 API auth tests; fixed 4 existing route tests (now send a supervisor identity).

## Streamlit identity + fail-loud hardening
- Streamlit now reads st.context.headers into resolve_auth_context (Entra identity
  resolves in the UI; forged headers ignored without trusted proxy).
- Pseudonym salt refuses the dev fallback in patient-data mode (predictable
  pseudonyms on real data = leak). Key Vault provider and durable audit sink now
  RAISE in patient-data mode if no client is wired (no silent downgrade / dropped
  audit). 9 hardening tests.

## Abuse protection
- app/security/rate_limit.py: per-user sliding-window rate limit, max prompt
  length, per-session case cap, repeated-block tracking; wired into the agent
  gateway boundary with clear UI messages. 9 tests.

## Wording + docs
- Reinforced "ML chooses the level; deterministic safety cross-check (escalation
  floor) adjusts; agents explain, not decide" as a user-visible caption.
- SECURITY_ARCHITECTURE.md: + FastAPI boundary, fail-loud hardening, abuse
  protection, env-var profile tables, session/retention/incident-response.
  DEPLOYMENT_SECURITY_CHECKLIST.md: + sections H (server-side/abuse), I (session/
  user-side), J (retention/incident response).

## Tests / build
- Full suite 572 -> 590 across the session (528 non-frontend + 62 frontend). New:
  test_mimic_full_adapter, test_api_auth_boundary, test_patient_data_hardening,
  test_rate_limit.

## Honest scope (unchanged)
- Public-data research demo. Full MIMIC stays on the credentialed environment
  (read via MIMIC_FULL_ED_DIR), never copied here. Not clinically validated;
  clinician review required on every output. FastAPI boundary is built + tested
  but the real Entra/Key Vault/durable-audit CLIENTS are wired in the deployment
  image, and a private-network Azure deployment + security review remain hospital/
  IT work.

## Genuinely Dylan's (cannot be done here)
- Run full-MIMIC schema validation + retraining on the credentialed machine;
  wire real Key Vault + durable audit clients; provision Entra/MFA/private
  network; deploy + security review; pytest in pinned 3.11.

---

# Session: v13 — final two-service architecture (one complete version)

Per the directive to produce ONE final complete version (not a staged roadmap),
this session implemented the full Streamlit-frontend + protected-FastAPI-backend
architecture with the backend as the sole server-side enforcement boundary, plus
the remaining hardening from the external review (items A–S), verified against the
code. The full credentialed MIMIC-IV-ED was NOT used here; only synthetic
MIMIC-shaped fixtures.

## Identifier leak fixed (items D/E/F) — the core security change
- make_case_uid now delegates to the pseudonymous HMAC function everywhere; raw
  source_dataset:stay_id is gone from all 13 call sites. URL-safe separator '~'
  (the earlier '#' was truncated in URL paths; '.' collided with version dots).
- New app/storage/guarded_writer.py: single safe write path. Redacts identifier
  keys (subject_id/stay_id/...), asserts no-raw-identifier, refuses local JSONL
  and requires the durable sink (fail closed) in patient-data mode. Workflow-run,
  human-review, and rerun repositories all route through it.
- HumanReviewRecord restructured: case_uid is an explicit persisted pseudonymous
  field (auto-derived), stay_id is Optional and redacted before persistence;
  reviewer identity fields added. Lookups match on stored case_uid.
- record_access now fails closed in patient-data mode (was except: pass).

## Canonical case_uid-keyed API (items A/J/L)
- app/api/case_routes.py: GET /cases, GET /cases/{case_uid}, POST
  /cases/{case_uid}/{assessments,explanations,reviews,followups}. RBAC-enforced,
  audited, no raw-id leakage; clinical content gated by can_view_clinical_content.
- app/api/case_resolver.py: maps pseudonymous case_uid -> case (recompute + match,
  dataset-scoped) since HMAC is one-way; honours the UI's case sources so the
  backend resolves exactly what the UI shows.
- app/api/status_routes.py: GET /security/status, /audit/events, /model/performance.
- app/security/security_status.py: single source of truth for posture + unsafe
  combinations; the startup guard and the endpoint both use it.

## Streamlit -> FastAPI HTTP refactor (the main directive)
- frontend/api_client.py: FASTAPI_BASE_URL two-service HTTP, else in-process ASGI
  (Starlette TestClient) against the same FastAPI app — ONE enforcement path, no
  Streamlit bypass. Forwards the Entra principal header; forwards the UI-selected
  demo role (X-Demo-Role / DEMO_ROLE env), ignored by the backend in patient-data
  mode.
- Rerouted protected actions through the backend: assessment run, review
  submission; audit-log and model-performance reads route through the backend in
  patient-data mode (read guard). Sidebar security-status panel added.

## Reviewer identity (item C)
- Patient-data mode: review form shows reviewer identity from auth (no dropdown);
  backend records reviewer_user_id/roles/auth_source; override_reason required for
  genuine overrides.

## Full-MIMIC scaffolding (item H) — credentialed env only
- ml_training/full_mimic/: _safety (require_safe_environment: needs
  MIMIC_FULL_ED_DIR outside repo, PATIENT_DATA_MODE, rejects repo paths, blocks raw
  rows), verify_schema, build_features, train, evaluate, reports (under/over-
  triage, high-acuity recall, calibration), model_card, dataset_card,
  check_artifact_compatibility. scripts/load_full_mimic_ed.py (old unsafe
  downloader into data/raw) DISABLED.

## Requirements split (item N) + docs (item R)
- requirements.txt (app), requirements-dev.txt (tests), requirements-ml.txt
  (training), requirements-azure.txt (Key Vault + durable audit). sklearn pinned.
- SECURITY_ARCHITECTURE.md updated with the v13 final two-service architecture.

## Tests / build
- Full suite 607 -> 622 (560 non-frontend + 62 frontend). New: test_case_routes_v13,
  test_api_client, test_full_mimic_scaffolding; many existing tests updated for the
  pseudonymous '~' format and the redacted-stay_id review schema.

## Honest scope (unchanged)
- Public-data research demo. Full MIMIC stays on the credentialed environment
  (MIMIC_FULL_ED_DIR), never in repo/zip/Docker/tests/CI. Not clinically validated;
  clinician review required on every output. The FastAPI boundary, redaction, and
  fail-closed behaviour are real and tested; the real Entra/Key Vault/durable-audit
  CLIENTS and the private-network Azure two-service deployment + security review
  remain hospital/IT work.

---

# Session: v13 review-driven hardening (verified against an external code review)

An external LLM review flagged 20 items / 7 blockers. Each was VERIFIED against the
actual code before action (per the project's verify-don't-agree discipline). Real
bugs were fixed; items that were already-handled, by-design, or environment-only
were noted and not "fixed" spuriously. Full credentialed MIMIC was never used.

## Confirmed-real bugs fixed (verified by reproduction first)
- **Nested raw-ID leak in the canonical API (blocker).** GET /cases, GET
  /cases/{uid}, and the assessment response returned the full case/workflow dump
  with nested subject_id/stay_id/hadm_id and retrospective data (diagnoses,
  disposition, medrecon, pyxis). Added app/api/safe_dto.py: allow-listed DTOs
  (safe_case_summary / safe_clinical_case_view / safe_assessment_response /
  safe_followup_response / safe_explanation_response) + a recursive identifier
  stripper and assert_no_raw_identifiers guard. Verified no raw IDs anywhere for
  both KTAS and MIMIC.
- **Follow-up acuity always None.** The route read top-level final_acuity; the
  workflow stores final_acuity_assessment.final_acuity. Fixed; verified a real
  3→1 escalation.
- **Full-MIMIC scaffolding imported a non-existent build_feature_frame.** Added
  build_feature_frame(cases) to ml_training/feature_engineering.py (accepts
  EDTriageCase objects or dicts). The scripts now run end-to-end on synthetic
  fixtures (new tests).
- **Raw f"Kaggle-KTAS:{sid}" case_uid in the review queue.** Replaced with the
  pseudonymous make_case_uid; removed raw stay_id from all queue rows/displays.
- **explain_case ignored the user question.** It now passes the question through
  the agent gateway (injection screen, rate limit, audit) before explaining.

## Architecture / enforcement (per directive: one protected path)
- **Legacy raw-ID routes removed from the default public surface.** /triage/*,
  /review/by-stay/*, /followup/history/*, /explain/llm/*, /chat/* are registered
  ONLY behind ALLOW_LEGACY_RAW_ID_ROUTES=true (default false) and are ALWAYS
  blocked in PATIENT_DATA_MODE (404), regardless of the flag. Verified all three
  states. The public API is case_uid-only.
- **Triage Review card routed through the backend.** In patient-data mode the
  workflow runs server-side only (no local run_workflow; card rendered from the
  safe DTO). Review Queue submission and the queue role handling now go through
  the backend (identity from auth, not a dropdown). The raw-stay_id two-visit
  follow-up path is blocked in patient-data mode.
- **/audit/events** reads the durable sink in patient-data mode (fails closed if
  only local JSONL is available).
- **CORS** check now parses comma-lists and rejects any '*' entry.
- **FASTAPI_BASE_URL required in patient-data mode** (API client returns 503
  unless ALLOW_IN_PROCESS_BACKEND_FOR_PATIENT_DATA=true for local dev).
- **Full-MIMIC model router**: MIMIC-IV-ED-Full-v2.2 → full model via
  MIMIC_FULL_MODEL_PATH; fails closed (unavailable/incompatible) and never
  substitutes the demo/KTAS model.

## Deployment artefacts → two-service (was "Option A — Streamlit-only")
- Dockerfile dispatches on SERVICE_ROLE (backend=uvicorn app.main:app,
  frontend=streamlit). New startup-backend.sh / startup-frontend.sh; the frontend
  script enforces FASTAPI_BASE_URL in patient-data mode.
- azure_preflight_check.py checks the two-service architecture + patient-data
  safety + legacy-routes-blocked; reports deployment_target=two_service_...
- app/main.py renamed to "AI Triage Research System" v13.0.0.
- README + infrastructure/azure_deploy.md updated; the stale Option A text is
  superseded. SECURITY_ARCHITECTURE.md already documents the final architecture.

## Items intentionally NOT "fixed" (verified as non-bugs)
- Real Key Vault / durable-audit CLIENTS not wired: by design (honest fail-closed
  seams); deployment-time concern, already documented.
- Sandbox sklearn/model-load failure: environment-only (pinned Python 3.11 vs the
  sandbox); the artefact-compatibility check already handles version mismatch.

## Tests / build-protocol learnings
- 622 → 640 (578 non-frontend + 62 frontend). New: comprehensive recursive
  no-raw-id across all endpoints, full-MIMIC scaffolding end-to-end on synthetic
  data, model-router fail-closed, CORS/FASTAPI_BASE_URL hardening.
- Added a session conftest fixture to generate the KTAS sample if absent (the
  suite no longer depends on another test generating it as a side effect).
- IMPORTANT: do NOT run scripts/run_ktas_pipeline.py in the working copy — it
  overwrites the committed combined model registry (KTAS+MIMIC) with a KTAS-only
  one and breaks model-dependent tests. Only build_sample_cases.py (sample) and
  build_ktas_labels.py (labels) are safe to regenerate; the registry is a
  committed artefact.

---

# Session: MIMIC-IV-ED-Full-only refactor (second external review, verified)

A second external review (10 blockers) was checked item-by-item against the code.
Confirmed-real bugs were fixed; institutional/infrastructure items were documented
as out-of-code-scope (never stamped "done"). In parallel, per an explicit project
decision, KTAS and MIMIC-demo were REMOVED from the live prediction/serving
system: full MIMIC-IV-ED (credentialed) is now the ONLY real prediction/training
path, with synthetic MIMIC-shaped fixtures for tests. The full credentialed MIMIC
dataset is never extracted in any shared environment.

## Architecture change: full-MIMIC-only, backend-only
- **Prediction router** (app/agents/ml_prediction_agent.py): deleted the KTAS and
  MIMIC-demo branches; the only model path is full MIMIC-IV-ED via
  MIMIC_FULL_MODEL_PATH, failing closed when absent/incompatible. Every other
  dataset returns no_model_for_dataset (no prediction). The deterministic safety
  layer still runs for all datasets and is what fires on critical physiology.
- **Case resolver** (app/api/case_resolver.py): KNOWN_DATASETS = ("mimic_full",);
  loads full MIMIC from MIMIC_FULL_ED_DIR on an approved environment, or a
  synthetic MIMIC-shaped frontend_cases_override.jsonl for tests. Demo/KTAS are no
  longer resolvable or servable.
- **Streamlit is frontend-only for protected actions.** The Triage Review and
  Follow-up tabs are fully backend-driven (assessment/explanation/review/followup
  via api_client → FastAPI). No local run_workflow on any protected path. The
  dataset selector is removed (one dataset = nothing to select); load_cases() now
  reads the backend (or returns empty), never the local KTAS/demo loaders, and no
  longer halts the app.
- **Model registry** is full-MIMIC-only: removed best_ktas_model,
  best_mimic_acuity_model, all_ktas_models, etc.; it now describes the full-MIMIC
  model slot (MIMIC_FULL_MODEL_PATH, not present in repo) and records the blocked
  leakage/outcome columns (acuity, disposition, outtime, hadm_id, diagnoses,
  medrecon, pyxis, vitals_timeseries).
- **Dockerfile** no longer bakes any dataset into the image (demo/KTAS copies
  removed); full MIMIC is read only from MIMIC_FULL_ED_DIR at runtime.

## Confirmed-real blockers from the review, fixed
- **Blocker 4** (st.stop() in the Triage tab killed all later tabs in patient-data
  mode): the tab now uses conditional rendering, not st.stop(); other tabs render.
- **Blocker 5** (audit / model-performance tabs called the backend then read local
  JSONL): in patient-data mode the audit tab renders FROM the backend durable
  sink; local JSONL is not the patient-data display source.
- **Blocker 6** (full MIMIC absent from the canonical API): the resolver now serves
  full MIMIC through /cases (verified end-to-end with synthetic fixtures).
- **Blocker 8** (stale docs): app/main.py docstring rewritten to the two-service
  architecture; README curl examples switched from legacy /chat + stay_id to the
  canonical /cases/{case_uid}/... API.
- **Blocker 9** (Dockerfile masked Azure dependency install failures with
  `|| true`): removed, so the build fails if Azure deps cannot install.

## Verified NON-code-fixable (documented, not "fixed")
Blockers 1, 2 (env half), 7, 10 — Entra/MFA/Conditional Access/private network/
managed-device wiring, real Key Vault + durable-audit clients, full-MIMIC model
training/evaluation/model-card, DUA/UHL governance, security/penetration test, and
clinical safety review. These are hospital/IT/credentialed-environment
deliverables. The codebase fails closed pending them; it is NOT, and is never
labelled, "patient-data deployment-ready".

## New: safety-first model comparison
- ml_training/full_mimic/compare_models.py (separate from the simple train.py):
  trains RandomForest + GradientBoosting (+ xgboost/lightgbm/catboost when
  installed; missing libs skipped, never fatal) and selects the best by
  TRIAGE-SAFETY metrics — high-acuity recall (primary), low under-triage rate
  (secondary), accuracy (tiebreaker only) — NOT accuracy. Runs only under
  require_safe_environment(); writes an aggregate comparison + the selected model
  artefact, never raw rows. Tested end-to-end on synthetic MIMIC-shaped data.

## Tests / isolation
- ~60 obsolete demo/KTAS-prediction tests were rewritten to assert the surviving
  deterministic-safety behaviour, or removed; security tests (recursive no-raw-id)
  re-pointed to synthetic full-MIMIC fixtures. Net suite: 581 passing
  (572 non-frontend + 9 frontend). Preflight PASS (two-service, full-MIMIC-only).
- Fixed a real test-isolation bug: test_main_cors reloaded app.config and orphaned
  the settings object, breaking downstream tests that monkeypatch
  app.config.settings. Fixed with module teardown + patching the resolver's own
  settings reference directly (the reload-resilient pattern).

---

# Session: KTAS hard-delete + full-MIMIC-only UI/backend + safety-first comparison

Per an explicit decision, KTAS and MIMIC-IV-ED Demo were HARD-DELETED from the live
tree (not just gated). The system is now full MIMIC-IV-ED only end to end. This
changelog retains history only; the live code contains no KTAS/demo runtime.

## Backend truth (/, /health, /cases, resolver)
- Added full_mimic_status() as the single source of truth (reads MIMIC_FULL_ED_DIR
  and MIMIC_FULL_MODEL_PATH live). / and /health now report datasets_available =
  ["MIMIC-IV-ED-Full-v2.2"], mimic_full_dir_configured, mimic_full_model_configured,
  patient_data_ready=false, clinical_use=NOT_FOR_CLINICAL_USE — no KTAS/demo.
- /cases serves full-MIMIC only via the resolver (KNOWN_DATASETS=("mimic_full",)),
  failing closed (empty) without MIMIC_FULL_ED_DIR; it never loads demo/KTAS.
- The legacy raw-stay_id review/triage endpoints now resolve via the full-MIMIC
  resolver only (no demo/KTAS loaders); they remain off by default and patient-
  data-blocked.

## Streamlit UI — full-MIMIC-only, no KTAS/demo text anywhere
- Sidebar System Status, page caption, module docstring, and no-cases messages
  rewritten to full-MIMIC-only.
- Model Performance rewritten to show ONLY full-MIMIC model status, the safety-
  first comparison report/CSV/model card if produced on the approved environment.
- Review Queue made backend-driven + full-MIMIC-only (no dataset selector, no
  local demo/KTAS queue).
- Governance policy checks refactored off demo/KTAS loaders to synthetic full-MIMIC
  cases; dataset cards and verdict strings rewritten.
- ~950 lines of orphaned dead helper code removed (the old local Triage card chain,
  _load_cases_cached, demo/KTAS imports).

## Hard-deleted from the live tree
- app/data_pipeline/ktas_adapter.py
- scripts/run_ktas_pipeline.py, build_ktas_labels.py, build_mimic_demo_labels.py,
  audit_mimic_demo.py, build_outcome_labels.py, build_sample_cases.py,
  inspect_missing_triage_inputs.py, download_mimic_ed_demo.py
- KTAS-specific test files; KTAS model artefacts (moved to archive/, excluded from
  packaging); raw KTAS + MIMIC-demo data; DUAL_PIPELINE_ARCHITECTURE.md,
  KTAS_SAFETY_NOTES.md
- load_mimic_demo_cases() removed from mimic_adapter.py (the shared MIMIC parser
  that the full-MIMIC loader depends on is retained)
- Dead KTAS branches removed from manchester_engine.py and
  followup_comparison_agent.py (the deterministic safety layer still flags critical
  physiology for all cases via the main path)
- Real KTAS defaults/advertising fixed (internal.py source_dataset defaults,
  governance_routes datasets)

## Proof tests (fail if KTAS/demo reappears in live code)
- tests/test_no_ktas_in_live_code.py: static guards (no ktas_adapter, Kaggle-KTAS,
  run_ktas_pipeline, RandomForest_ktas, mimic_demo_acuity_rf, demo/KTAS loaders in
  app/frontend/scripts — archive/ and this changelog excluded) + behavioural proofs
  (/, /health full-MIMIC-only; router never predicts demo/KTAS; /cases full-MIMIC-
  only; no live import of demo/KTAS loaders).
- test_frontend.py additions: rendered UI text contains no KTAS/MIMIC-IV-ED Demo
  across roles; frontend calls api_client and never run_workflow directly.

## compare_models.py upgrade
- Candidates: Logistic Regression, Random Forest, Gradient Boosting, + XGBoost/
  LightGBM/CatBoost when installed (missing libs skipped, never fatal).
- Safety-first selection: high-acuity recall, then low under-triage, then accuracy.
- Outputs: JSON + CSV comparison, per-class precision/recall/F1, macro/weighted F1,
  confusion matrix per candidate, train/infer timing, selection rationale, model
  card. No raw rows in any output.
- --quick-test (or MIMIC_COMPARE_QUICK=1) shrinks estimators so tests finish in
  ~2s (previously the synthetic test could stall >180s).
- Fixed a real bug: CatBoost predictions are 2D (n,1); flattened so CatBoost is
  evaluated rather than skipped.

Net suite: 596 passing (582 non-frontend + 14 frontend). Preflight PASS
(two-service, full-MIMIC-only).

---

# Research checkpoint: full-MIMIC loading fix (NOT deployment-ready)

This checkpoint fixes the loading bug that prevented trained credentialed full
MIMIC-IV-ED from appearing in the app. It does NOT address the larger model-
development/security items (tracked in priority order 1-7; see
LOADING_CHECKPOINT_PROOF_STATUS.md). Test count is the author's local result and
is NOT independently verified in this checkpoint.

Two bugs, different stages:
- Loader/profile (actual cause of empty case list): full MIMIC required
  PATIENT_DATA_MODE=true; in local demo mode that is false, so the loader refused
  and the resolver/frontend swallowed the error and returned [].
- Orchestrator (downstream second failure): final acuity built only for the
  deleted MIMIC-IV-ED-Demo-v2.2 dataset, so a loaded full-MIMIC case showed no
  final acuity. Fixed to MIMIC-IV-ED-Full-v2.2.

Changes:
- NEW LOCAL_CREDENTIALED_RESEARCH profile (app/security/identity.py): distinct
  from production PATIENT_DATA_MODE, mutually exclusive (production wins). Lets a
  credentialed researcher load their own MIMIC locally. Enforces DATA guards
  (path set/outside-repo/exists/is-dir/core tables present). Does NOT yet enforce
  loopback bind or block cloud calls (priority 1).
- mimic_full_loader.py: _assert_safe_to_use() now allows either credentialed
  profile and raises SPECIFIC reasons (env not set / mode not enabled / inside
  repo / not exist / not dir / missing core tables = wrong dir level). Added
  full_mimic_diagnostic() returning a non-sensitive reason (never the path).
- case_resolver.py: logs the specific reason instead of silently returning [].
- health_routes.py: new GET /status/full-mimic diagnostic endpoint.
- api_client.full_mimic_status() + sidebar now read status FROM THE BACKEND
  (authoritative in two-service mode), not the Streamlit container's env.
- orchestrator.py: final-acuity branch fixed to MIMIC-IV-ED-Full-v2.2.
- tests/test_full_mimic_end_to_end.py: profile/config -> loader -> /cases -> case
  selection -> assessment -> visible final acuity; production precedence; missing-
  env and wrong-dir-level diagnostics; demo no longer yields final acuity.
- Updated loader-guard tests (seam/adapter) to the new profile gate + schema guard.

Honest proof status (see LOADING_CHECKPOINT_PROOF_STATUS.md):
A diagnostic / B /cases pseudonymous / C visible acuity / D no-silent-swallow:
present in this build. E loopback-only bind / F cloud-block: NOT YET IMPLEMENTED
(priority 1).
