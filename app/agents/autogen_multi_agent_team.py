...
from app.agents.autogen_team import (
    _safe_close_model_client,
    AutoGenDependencyError,
    ENV_PATH,
    _make_evidence_tool,
    build_model_client,
    load_azure_config,
...
DEFAULT_CATEGORY_REASONING_REQUEST = (
    "Explain why the already-computed acuity/category estimate was shown. "
    "Focus on decision-driving evidence only: chief complaint/pathway, "
...
def _autogen_dependency_unavailable_result() -> dict[str, Any]:
    return {
        "status": "NOT_CONFIGURED",
        "agent_turns": [],
        "final_explanation": (
            "The multi-agent explanation layer is not available in this runtime "
            "because its AutoGen packages are not installed. Human clinical "
            "review of the verified evidence is still required."
        ),
        "safety_failures": ["AUTOGEN_RUNTIME_NOT_INSTALLED"],
    }
...
once for the case_uid given in the task, then identify the case facts that could
explain the already-computed acuity/category estimate. Focus on chief complaint,
arrival mode, abnormal vitals, missing vitals, safety flags, and the
category_reasoning.primary_drivers field. Do NOT list every vital sign when it
is normal and not a driver.
...
5. If a symptom could be associated with serious conditions, describe it only as
   a high-risk presentation or pathway signal; do not name a diagnosis unless
   the verified evidence itself contains that diagnosis.
"""
...
You have exactly one tool: get_verified_evidence_for_case(case_uid). Call it
once for the case_uid given in the task, then report only data limitations that
affect confidence in the already-computed estimate. If no required field is
missing, say that briefly and move on; do not turn this into a full case
summary.
...
is to explain which already-returned rule/safety signals support the displayed
estimate: rules_engine_status, rules_engine_reason_codes, safety_flags,
is_safe_to_present, final_acuity_assessment.override_* fields, and
category_reasoning.rules_summary. Mention normal vitals only to say they did
not trigger a critical-vital override.
...
clinician-facing answer to: "Why did the system show this exact acuity/category
estimate?" Do not call the evidence tool yourself unless none of the previous
agents did. Use category_reasoning.primary_drivers and
category_reasoning.model_probability_summary when they are available.
...
9. LENGTH IS A HARD LIMIT. Write FIVE SENTENCES OR FEWER as a single plain
   ...
   renders plain text. Do not restate evidence the reader can already see on
   screen. If you cannot fit something, drop the least decision-relevant
   detail rather than exceeding five sentences.
10. End your summary with the word TERMINATE on its own line.
"""
...
# Must match an AFFIRMATIVE, FORWARD-LOOKING requirement for clinician review.
# Matching on the mere presence of "clinician" and "review" accepted the exact
# opposite of the requirement: "No clinician review is required.", "The clinician
# reviewed this yesterday." and "The clinician did not review the case." all
# satisfied it. A past-tense or negated statement does not tell the reader that
# review is still needed, which is the whole point of the mandated sentence.
_REVIEW_SENTENCE_RE = _re_module.compile(
    r"(?<!\bno\s)(?<!\bnot\s)"
    r"\bclinician(?:'s)?\s+(?:review|confirmation|sign[- ]?off)\s+"
...
_REVIEW_NEGATION_RE = _re_module.compile(
    r"\b(?:no|not|never|without|n't)\b", _re_module.IGNORECASE
)


def _is_affirmative_review_sentence(sentence: str) -> bool:
    """True only for a present/future affirmative clinician-review requirement."""
    if not _REVIEW_SENTENCE_RE.search(sentence):
        return False
    return not _REVIEW_NEGATION_RE.search(sentence)


def condense_explanation(text: str, max_sentences: int = MAX_EXPLANATION_SENTENCES,
    ...
    It only strips presentation (markdown emphasis, bullet/heading markers, the
    TERMINATE sentinel) and truncates whole trailing sentences. It never
    rewrites, reorders, or paraphrases clinical content, so a reader can always
    trust that what is shown was actually generated from the verified evidence.
    """
    if not text:
        return ""
    cleaned = str(text).replace("TERMINATE", " ")
    cleaned = re.sub(r"\*\*|__|`", "", cleaned)              # bold / code emphasis
    cleaned = re.sub(r"(?m)^\s*#{1,6}\s*", "", cleaned)       # markdown headings
    cleaned = re.sub(r"(?m)^\s*[-*\u2022]\s+", "", cleaned)   # bullet markers
    cleaned = re.sub(r"\s+", " ", cleaned).strip()
    # Inline "- Label:" separators the model uses when it ignores rule 9 and
    # inlines its section headings into one paragraph.
    ...
    # validator requires, turning a valid explanation into
    # MISSING_HUMAN_REVIEW_REQUIREMENT and withholding it from the clinician.
    review_idx = next(
        (i for i in range(len(parts) - 1, -1, -1)
         if _is_affirmative_review_sentence(parts[i])),
        None,
    )
    review_sentence = parts.pop(review_idx) if review_idx is not None else None

    budget = max(1, int(max_sentences)) - (1 if review_sentence else 0)
    kept = parts[:max(0, budget)]
    out = " ".join(kept).strip()
    ...
    # The prompt requires the explanation to END with the clinician-review
    # statement, and the downstream validator REQUIRES that statement to be
    # present. Dropping trailing sentences therefore deleted the exact sentence
    # that makes the explanation valid: a perfectly good explanation became
    # MISSING_HUMAN_REVIEW_REQUIREMENT and was withheld entirely. Availability
    # matters here -- a clinician who gets nothing is worse off than one who
    # gets a slightly shorter summary. So the review sentence is lifted out,
    # the body is trimmed against the space that remains, and it is put back.
    reserved = (len(review_sentence) + 1) if review_sentence else 0

    while len(out) + reserved > limit and len(kept) > 1:
        kept = kept[:-1]
        out = " ".join(kept).strip()

    if review_sentence and len(out) + reserved > limit:
        # Even one body sentence plus the review statement will not fit; the
        # review statement wins, because it is the mandatory part.
        kept = []
        out = ""

    if review_sentence:
        # Exempting this sentence from truncation created an unbounded hole: a
        # 2,678-character "review" sentence was returned whole, defeating the
        # cap entirely. Protection means it is never DROPPED, not that it may be
        # any length. If it alone exceeds the budget it is replaced with a fixed
...
async def run_team_explanation(
    ...
    except AutoGenDependencyError:
        return _autogen_dependency_unavailable_result()
    ...
    try:
        ...
        final_explanation = condense_explanation(final_explanation)
        ...
        await _safe_close_model_client(model_client)
...
async def run_case_uid_team_explanation(
    ...
    try:
        ...
        requested_focus = (
            ...
            else DEFAULT_CATEGORY_REASONING_REQUEST
        )
        task += f"Clinician-requested focus: {requested_focus}"
        try:
            result = await team.run(task=task)
        except Exception as exc:
            return {
