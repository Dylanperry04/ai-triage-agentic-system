"""
Real multi-agent AutoGen team: IntakeAgent, ValidationAgent,
SafetyReviewAgent, ExplanationAgent, coordinated via RoundRobinGroupChat.

WHY THIS MODULE EXISTS, READ BEFORE EDITING
=============================================
app/agents/autogen_team.py already has a working single AssistantAgent
for the clinician chat tab. That design is sound and is NOT being
replaced here -- this module is an ADDITIONAL capability for cases where
a structured, multi-stage explanation walkthrough is wanted (e.g. the
Triage Review tab's assessment card), built using AutoGen's own
RoundRobinGroupChat team primitive instead of a single agent, because a
single AssistantAgent is not what "multi-agent orchestration" means in
the AutoGen ecosystem -- RoundRobinGroupChat (or another real Team
subclass) is.
...
just replicated across four agents instead of one:
  ...
  2. EACH AGENT'S SYSTEM MESSAGE NARROWS ITS ROLE, NOT WIDENS ITS
     ...
     KTAS-Manchester mapping, must state human review is required).
...
2. Do NOT create, assign, infer, or change a Manchester or KTAS category \
...
6. Do NOT map or convert KTAS to Manchester in any way.
...
async def run_team_explanation(
    ...
    """
    cases_path = cases_path or (settings.processed_dir / "triage_cases_sample.jsonl")
    ...
    if config is None:
        return {
            ...
            "final_explanation": (
                ...
                "AZURE_OPENAI_API_VERSION in .env to enable the multi-agent "
                "team. Human clinical review of the verified evidence is "
                "still required."
            ),
            "safety_failures": [],
        }
