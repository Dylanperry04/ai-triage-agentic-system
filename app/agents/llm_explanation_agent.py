...
If a clinician question is present, answer that question first. Do not dump all
available patient facts. Mention a vital sign, demographic, or field only when
it directly answers the question, was missing/abnormal, contributed to a rule or
model estimate, or explains why it did not drive the estimate.

For a clinician question, your response must have exactly these five sections:
1. Direct Answer
2. Evidence Relevant To The Answer
3. Missing or Limited Data
4. Safety / ML / Rules Context
5. Human Review Required — Action Needed
"""


def _validate_explanation_structure(text: str) -> list[str]:
    """Structured validation of the five-section response format.

    The explanation agent can answer either a direct clinician question or a
    general category-reasoning request. This verifies numbered sections and the
    required concepts without forcing one exact heading set.
    """
    failures: list[str] = []
    import re

    heading_matches = re.findall(r"(?im)^\s*([1-5])[\.\)]\s*([^\n:]{0,120})", text)
    present_nums = {int(num) for num, _heading in heading_matches}
    ...
    headings = " ".join(heading for _num, heading in heading_matches).lower()
    concept_checks = [
        ("MISSING_SECTION_DIRECT_ANSWER_OR_EVIDENCE",
         r"(direct\s+answer|category\s+reasoning|evidence)"),
        ("MISSING_SECTION_MISSING_DATA", r"(missing|limited)"),
        ("MISSING_SECTION_SAFETY_ML_RULES", r"(safety|ml|rules|risk)"),
        ("MISSING_SECTION_HUMAN_REVIEW", r"(human\s+review|action)"),
    ]
    for failure_name, pattern in concept_checks:
...
def _build_prompt(case_evidence: dict[str, Any],
    ...
    focus_instruction = (
        "Explain why the already-computed category or ML estimate was shown. "
        "Do not list every vital sign or demographic unless it was abnormal, "
        "missing, contributed to the estimate, triggered a rule, or explicitly "
        "explains why it did not drive the estimate."
    )
...
"""
        question_response_guidance = """
Because a clinician question is shown, use this response format instead of a
...
"""
        focus_instruction = (
            "Answer the clinician question first. Keep the answer focused on "
            "the reason or evidence requested. Do not recap all patient facts."
        )
    return f"""
