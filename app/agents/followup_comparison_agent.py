...
def compare_follow_up(
    ...
    # (smaller number = more urgent => escalation). Same-dataset only; a
    # cross-dataset pair already carries dataset_warning and we do not compute a
    # movement for it. Retired non-full-MIMIC class comparison is intentionally
    # not used.
    ...
    if same_dataset and str(prev_ti.source_dataset or "").startswith("MIMIC-IV-ED"):
    ...
    # (The KTAS-class comparison branch was removed: KTAS is not a prediction
    #  source and predicted_ktas_class is never populated.)
                category_movement = "NO_CHANGE"
