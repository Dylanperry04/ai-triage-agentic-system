...
def run_workflow(
    ...
    # Not applicable to KTAS (no MTS category) or when no acuity prediction ran.
    from app.schemas.workflow import FinalAcuityAssessment
    final_acuity_assessment = FinalAcuityAssessment()
    if (
        triage_input.source_dataset == "MIMIC-IV-ED-Demo-v2.2"
    ...
    audit = {
        ...
        "ml_policy": (
            "ML predictions are dataset-specific and never mixed. For "
            "MIMIC-IV-ED Demo cases, an acuity model predicts the ESI acuity "
            "level, which is mapped to a five-level colour/priority display; a "
            "deterministic escalate-only vital override may raise (never lower) "
            "the result for extreme/critical vitals. For Kaggle KTAS cases, a "
            "KTAS model predicts a KTAS class only and no Manchester/MTS category "
            "is shown. The KTAS model is never applied to MIMIC and vice versa. "
            "All ML outputs are research-grade only and do not replace "
            "deterministic safety review or clinician judgement."
        ),
        "safety_guardrail": (
