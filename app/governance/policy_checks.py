...
def run_policy_checks(run_workflow_fn: Callable, settings=None,
    ...
    """Run the executable policy checks for the active UHL serving path.

    Each check runs a real invariant. The data-dependent checks use UHL-shaped
    synthetic cases. Extra positional args
    ...
    def _synth(hr=80.0, o2=98.0, sbp=120.0, cc="CHEST PAIN", acuity=None):
        return EDTriageCase(**{
            ...
            "stay_id": 1, "subject_id": 1,
            "edstay": {"subject_id": 1, "stay_id": 1, "gender": "F",
                       "intime": "2026-08-10 12:00:00",
            ...
            "triage": {"subject_id": 1, "stay_id": 1, "heartrate": hr, "o2sat": o2,
                       ...
                       "age": 50, "pain": "5", "chiefcomplaint": cc, "acuity": acuity},
    ...
    try:
        wf = run_workflow_fn(_synth(hr=195.0, o2=80.0, sbp=70.0, cc="COLLAPSE / FAINT"))
    ...
    try:
        all_req = all(run_workflow_fn(_synth(cc=cc)).decision.requires_clinician_review
                      for cc in ("CHEST PAIN", "EYE PROBLEM", "INJURY"))
        checks.append(_check(
    ...
    # Policy 4: UHL labels/outcomes are declared blocked from serving features.
    try:
        import json
        ...
        if not label_blocked:
            ...
            msg = "UHL registry blocks label/outcome columns; no retired datasets."
