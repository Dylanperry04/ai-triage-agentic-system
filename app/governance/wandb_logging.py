...
def _load_wandb_sdk() -> Tuple[Optional[ModuleType], str]:
    """Return the real W&B SDK module, or a precise reason it is unusable."""
    try:
        import wandb
    except Exception as exc:
        return None, f"wandb import failed: {type(exc).__name__}: {exc}"
    if not hasattr(wandb, "init"):
        origin = getattr(wandb, "__file__", None) or "unknown location"
        return (
            None,
            (
                "imported module named 'wandb' is not the W&B SDK "
                f"(missing init; origin: {origin})"
            ),
        )
    return wandb, ""


def wandb_sdk_status() -> Dict[str, Any]:
    sdk, reason = _load_wandb_sdk()
    return {
        "available": sdk is not None,
        "reason": reason,
        "module_file": getattr(sdk, "__file__", None) if sdk is not None else None,
    }


def wandb_available() -> bool:
    return _load_wandb_sdk()[0] is not None
...
def log_governance_run(
    ...
    if local_credentialed_research_mode():
        ...
        try:
            ...
            pass
    wandb, wandb_reason = _load_wandb_sdk()
    if wandb is None:
        return {"status": "SKIPPED", "reason": wandb_reason or "wandb SDK not available"}
