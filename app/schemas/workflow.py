...
class MLPredictionResult(BaseModel):
    ...
    #   "KTAS"                         -> KTAS model output (KTAS cases)
    #   "MIMIC_ACUITY_MAPPED_TO_MTS"   -> MIMIC acuity model output (MIMIC cases),
    #                                     with the acuity mapped to MTS-style display
...
class FinalAcuityAssessment(BaseModel):
    ...
    populated ONLY for MIMIC cases (where an acuity model runs); it stays empty
    for KTAS (which shows KTAS output, not an MTS category) and when no ML
