...
class MLPredictionResult(BaseModel):
    ...
    # predicted_mimic_acuity is the acuity AFTER the artefact's decision rule.
    # When that rule is not plain argmax the assigned class is NOT the most
    # probable class, so top_class_confidence (max over all classes) belongs to
    # a DIFFERENT class than the one displayed. These fields let a caller show
    # the two numbers honestly instead of pairing an acuity with another
    # class's probability.
    argmax_acuity: Optional[int] = None            # most probable class
    argmax_probability: Optional[float] = None     # its probability (== top_class_confidence)
    assigned_acuity_probability: Optional[float] = None  # P(predicted_mimic_acuity)
    decision_rule_type: Optional[str] = None       # "argmax" | "high_acuity_threshold" | ...
    decision_rule_threshold: Optional[float] = None
    decision_rule_threshold_source: Optional[str] = None   # "artefact" | "deployment_override"
    artefact_decision_rule_threshold: Optional[float] = None
    decision_rule_changed_prediction: bool = False # True when assigned != argmax
    ...
    model_note: str = (
        "Credentialed research acuity model not loaded. Not for clinical use; "
