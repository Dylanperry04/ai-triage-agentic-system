...
class ScanUploadMetadata(BaseModel):
    """Safe metadata for an optional supporting file.

    The binary file is not stored in the audit log and is not sent to an LLM or
    multimodal model by this schema. When bytes_stored=True, storage_ref points
    to a local guarded upload path outside the audit record.
    """
    ...
    analysis_status: str = "metadata_recorded_pending_multimodal_analysis"
    bytes_stored: bool = False
...
class WorkflowRerunRecord(BaseModel):
    ...
    previous_chiefcomplaint: Optional[str] = None
    new_chiefcomplaint: Optional[str] = None
    updated_context: Optional[str] = None
    scan_uploads: List[ScanUploadMetadata] = Field(default_factory=list)
