...
def valid_stay_id_to_dataset() -> dict[int, str]:
    # Returns a stay_id -> source_dataset mapping, not just a bare set
    # of valid stay_ids, so callers can populate
    # HumanReviewRecord.source_dataset without needing to re-read this
    # file a second time. Added during a later review pass alongside
    # the new source_dataset field on HumanReviewRequest/HumanReviewRecord
    # (see app/schemas/review.py) -- stay_id alone is not a safe
    # identifier once multiple datasets exist.
    ...
    return {
        int(record["stay_id"]): record.get("source_dataset", "Unknown")
...
def submit_human_review(request: HumanReviewRequest):
    stay_id_to_dataset = valid_stay_id_to_dataset()
    if request.stay_id not in stay_id_to_dataset:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid stay_id: {request.stay_id}. Review can only be saved for a processed ED stay.",
        )

    # The server-side lookup (from the actual processed cases file) is
    # authoritative, not the client-supplied request.source_dataset --
    # a client could be wrong or could omit it entirely (it is Optional,
    # for backward compatibility with callers written before this field
    # existed). request.source_dataset is only used as a fallback if the
    # server-side lookup somehow returns "Unknown" for a stay_id that is
    # otherwise valid.
    resolved_source_dataset = stay_id_to_dataset.get(request.stay_id, "Unknown")
    if resolved_source_dataset == "Unknown" and request.source_dataset:
        resolved_source_dataset = request.source_dataset
