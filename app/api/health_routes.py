...
def _report_dir() -> tuple[object, bool]:
    ...
    return settings.uhl_report_dir, bool(os.environ.get("UHL_REPORT_DIR"))


def _uhl_status_payload():
    import hashlib
    from app.config import settings
    from app.constants import DATASET_SOURCE

    data_present = settings.uhl_data_path.is_file()
    model_present = settings.uhl_model_path.is_file()
    data_hash_verified = False
    model_hash_verified = False
    try:
        data_hash_verified = data_present and hashlib.sha256(
            settings.uhl_data_path.read_bytes()
        ).hexdigest() == settings.expected_dataset_sha256
    except OSError:
        pass
    try:
        model_hash_verified = model_present and hashlib.sha256(
            settings.uhl_model_path.read_bytes()
        ).hexdigest() == settings.expected_model_sha256
    except OSError:
        pass
    return {
        "dataset_source": DATASET_SOURCE,
        "dataset_ready": bool(data_hash_verified),
        "model_ready": bool(model_hash_verified),
        "data_hash_verified": bool(data_hash_verified),
        "model_hash_verified": bool(model_hash_verified),
        "active_case_source": DATASET_SOURCE if data_hash_verified else "not_configured",
        "active_case_source_mode": "packaged_uhl",
...
@router.get("/status/uhl")
@router.get("/status/full-mimic")
def uhl_status_endpoint():
    """Non-sensitive status for the active UHL data/model assets.

    The old route remains as a compatibility alias for an already-built 22.4
    browser bundle; both routes now report UHL only.
    """
    return _uhl_status_payload()
...
def runtime_status_endpoint():
    ...
    uhl = _uhl_status_payload()
    report_files = {
        "comparison_report_present": "uhl_synthetic_model_comparison.json",
        "dataset_card_present": "uhl_synthetic_dataset_card.json",
        "feature_schema_present": "uhl_synthetic_feature_schema.json",
        "training_provenance_present": "uhl_synthetic_training_provenance.json",
        "selection_record_present": "selection_record.json",
    }
    reports = {
        "env_present": bool(os.environ.get("UHL_REPORT_DIR")),
        "path": "redacted",
        **{
            key: (settings.uhl_report_dir / name).is_file()
            for key, name in report_files.items()
        },
    }
    try:
        from app.api import case_resolver
        case_count = case_resolver.count_cases("uhl") if uhl["dataset_ready"] else None
    except Exception:
        case_count = None
    sweeper_raw = os.environ.get("ENABLE_OVERDUE_VITALS_SWEEPER")
    sweeper_enabled = (
        sweeper_raw.lower() == "true"
    ...
    return {
        ...
        "package_checkpoint": PACKAGE_CHECKPOINT,
        "active_profile": "uhl",
        "active_case_source": uhl["active_case_source"],
        "active_case_source_mode": uhl["active_case_source_mode"],
        "uhl": {**uhl, "case_count": case_count},
        "model": {
            "env_present": bool(os.environ.get("UHL_MODEL_PATH")),
            "path": "redacted",
    ...
    # Unreachable 22.4 MIMIC diagnostic retained for source-history only.
    import hashlib
    import os
...
def assessment_cache_key_endpoint():
    ...
    from app.config import settings
    from app.constants import FEATURE_SCHEMA_HASH

    material = {
        "backend_version": APP_VERSION,
        "package_checkpoint": PACKAGE_CHECKPOINT,
        "feature_schema_hash": FEATURE_SCHEMA_HASH,
        "model_component": f"model-sha:{settings.expected_model_sha256}",
    }
    ...
    # Unreachable 22.4 MIMIC cache-key code retained for source history.
