...
# The only live dataset is the packaged UHL synthetic triage-vitals cohort.
KNOWN_DATASETS = ("uhl",)

from app.constants import DATASET_SOURCE as UHL_DATASET_LABEL
FULL_MIMIC_LABEL = "MIMIC-IV-ED-Full-v2.2"  # inactive compatibility label
...
def _dataset_may_have_uid_prefix(dataset: str, prefix: str) -> bool:
    ...
    """
    if dataset == "uhl":
        return prefix == UHL_DATASET_LABEL
    if dataset == "mimic_full":
...
_UHL_REPOSITORY = None


def _uhl_repository():
    global _UHL_REPOSITORY
    if _UHL_REPOSITORY is None:
        from app.data_pipeline.uhl_repository import UhlCaseRepository
        _UHL_REPOSITORY = UhlCaseRepository(settings)
    return _UHL_REPOSITORY
...
def _cache_signature(dataset: str) -> Any:
    ...
    if dataset == "uhl":
        try:
            st = settings.uhl_data_path.stat()
...
def _get_resolved_cases(dataset: str) -> List[ResolvedCase]:
    ...
    for case in _load_dataset_cases(dataset):
        ...
        uid = case.get("case_uid") or pseudonymous_case_uid(label, stay_id)
...
def _load_dataset_cases(dataset: str, n: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    if dataset == "uhl":
        # UI and search reads stay bounded.  The defensive cap on the legacy
        # unbounded caller avoids materialising all 777k rows in process memory.
        limit = int(n) if n is not None else 1000
        return _uhl_repository().list_cases(
            offset=0,
            limit=max(1, limit),
        )["cases"]
    if dataset != "mimic_full":
...
def _count_dataset_cases(dataset: str) -> int:
    if dataset == "uhl":
        return int(_uhl_repository().status()["source_rows"])
    sig = _cache_signature(dataset)
...
def resolve(case_uid: str) -> Optional[ResolvedCase]:
    ...
    """
    # UHL identifiers are already SHA-derived by the validated repository and
    # have a direct SQLite index, so resolution never scans the full cohort.
    payload = _uhl_repository().get_case(case_uid)
