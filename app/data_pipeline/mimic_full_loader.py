...
def active_case_source_status() -> dict:
    """Return the non-sensitive active case source labels used by status APIs."""
    diag = full_mimic_diagnostic()
    mode = str(diag.get("data_source_mode") or diag.get("active_profile") or "")
    active_source = "not_configured"
    if mode == "synthetic_supervisor_demo":
        active_source = "MIMIC-IV-ED-Synthetic-Supervisor-Demo"
    elif mode == "credentialed_mimic_azure_demo" or diag.get("full_mimic_loadable"):
        active_source = "MIMIC-IV-ED-Full-v2.2"
    return {
        "configured_prediction_dataset": "MIMIC-IV-ED-Full-v2.2",
        "active_case_source": active_source,
        "active_case_source_mode": mode or "unknown",
    }
...
def _load_mimic_table_rows_for_stays(path, table_name: str, stay_ids: set[int]):
    """Stream one MIMIC table and keep only rows matching selected stay IDs."""
    import csv
    import gzip
    import pandas as pd
    ...
    for table_path in candidate_mimic_table_paths(path, table_name):
        if not table_path.is_file():
            continue
        opener = gzip.open if table_path.name.endswith(".gz") else open
        rows = []
        with opener(table_path, "rt", encoding="utf-8", newline="") as f:
            reader = csv.DictReader(f)
            for row in reader:
                try:
                    sid = int(float(row.get("stay_id", "")))
                except (TypeError, ValueError):
                    continue
                if sid not in stay_ids:
                    continue
                rows.append(row)
                if len(rows) >= len(stay_ids):
                    break
        return pd.DataFrame(rows)

    # Unit tests monkeypatch load_mimic_table instead of writing a physical
    # triage file. Fall back to the adapter path so those tests can still assert
    # row-selection behaviour; in real serving this raises the same missing-file
    # error if the table truly is absent.
    from app.data_pipeline.mimic_adapter import load_mimic_table
    df = load_mimic_table(path, table_name)
    if "stay_id" not in df.columns:
        return df.iloc[0:0].copy()
    stay_col = pd.to_numeric(df["stay_id"], errors="coerce")
    return df.loc[stay_col.isin(stay_ids)].copy()


def load_mimic_full_cases(n=None):
    """
    Load cases from the FULL credentialed MIMIC-IV-ED, labelled as full (not
    demo), after all safety guards pass. This runs ONLY on the credentialed
    environment (local credentialed research or patient-data mode, path outside
    repo); it fails closed otherwise. Cases are tagged
    source_dataset='MIMIC-IV-ED-Full-v2.2' so they
    are never mixed with demo/KTAS data.

    Loads ALL six tables. Use this for TRAINING/feature building. For SERVING
    (listing/resolving triage-time cases), prefer load_mimic_full_cases_triage_time()
    which loads only the triage-time tables.
    """
    path = _assert_safe_to_use()
    from app.data_pipeline.mimic_adapter import (
        load_mimic_table, dataframe_to_cases, SOURCE_DATASET_LABEL_FULL,
    )
    dfs = {t: load_mimic_table(path, t)
           for t in ["edstays", "triage", "vitalsign", "diagnosis", "medrecon", "pyxis"]}
    return dataframe_to_cases(
        dfs["edstays"], dfs["triage"], dfs["vitalsign"],
...
def load_mimic_full_cases_triage_time(n=None):
    ...
    if nrows is not None and "stay_id" in edstays.columns:
        # Do not assume the first n rows of triage.csv.gz correspond to the first
        # n rows of edstays.csv.gz. Select triage rows by stay_id, but stream the
        # table so page serving does not materialise all full-MIMIC triage rows.
        selected_stays = set(
        ...
        triage = _load_mimic_table_rows_for_stays(path, "triage", selected_stays)
    else:
