...
# are not patient identifiers, so they must survive redaction exactly.
SAFE_WORKFLOW_TIMESTAMP_KEYS = {
    ...
    "escalation_closed_at",
    "discharged_at",
    "last_vitals_checked_at",
    "last_vitals_updated_at",
    "overdue_vitals_alert_created_at",
    "overdue_vitals_acknowledged_at",
...
SAFE_TIMESTAMP_SUFFIXES = ("_at", "_utc", "_timestamp")
...
def redact_text(value: str) -> str:
    ...
    phone-like numbers, dates, and long digit runs. Name-like capitalized word
    pairs are deliberately not redacted because the active datasets do not carry
    patient names and that heuristic damages clinical text. Explicit identifier
    keys such as name/first_name/last_name are still dropped by redact_for_log().
    Order matters (SSN/date before generic phone)."""
...
def redact_for_log(data: Dict[str, Any], *, extra_keys: Optional[Iterable[str]] = None,
    ...
    def _is_safe_metadata_string_key(key: Optional[str]) -> bool:
        if not key:
            return False
        key = key.lower()
        if key in SAFE_METADATA_STRING_KEYS or key in SAFE_WORKFLOW_TIMESTAMP_KEYS:
            return True
        if key in IDENTIFIER_KEYS or key in FREE_TEXT_KEYS:
            return False
        return key.endswith(SAFE_TIMESTAMP_SUFFIXES)
