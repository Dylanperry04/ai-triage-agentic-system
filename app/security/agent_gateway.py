...
def authorise_agent_call(
    ...
    - Authorisation: requires ``required_permission`` (defaults to
      can_ask_chatbot for the ITD free-text assistant). The clinician
      multi-agent case-acuity explanation passes PERM_EXPLAIN_CASE_ACUITY so it
      is authorised for clinical roles without granting them the ITD chatbot.
    ...
    """
    auth_perm = required_permission or authz.PERM_ASK_CHATBOT
    # 1. Authorisation.
    if not authz.has_permission(ctx, auth_perm):
        record_access(action=action, decision="DENIED", ctx=ctx, page=page,
                      case_uid=case_uid, permission=auth_perm,
                      detail=f"role lacks {auth_perm}")
