...
class EncryptedDurableAuditSink:
    ...
    def write(self, record: Dict[str, Any]) -> bool:
        ...
        # Canonical query timestamp: server-side reads filter on timestamp_utc,
        # but workflow-state records natively carry updated_at_utc only. In Azure
        # Tables a comparison on a MISSING property excludes the entity, so
        # without this stamp every workflow-state row would vanish from bounded
        # patient-mode reads. Stamp it (from the record's own clock if present).
        if not safe.get("timestamp_utc"):
            safe["timestamp_utc"] = str(
                safe.get("updated_at_utc")
                or safe.get("created_at_utc")
    ...
    def read_recent(
        ...
        *,
        ...
        """
        max_limit = _max_audit_read_limit()
...
def _audit_read_query_filter(*, record_kind: Optional[str], since_utc: str) -> str:
    ...
    if since_utc:
        ...
        # Legacy workflow-state entities predate the write-time timestamp stamp
        # and only carry updated_at_utc; match either so they are not dropped.
        clauses.append(
            f"(timestamp_utc ge '{safe_since}' or updated_at_utc ge '{safe_since}')"
        )
...
def _max_audit_read_scan(limit: int) -> int:
    ...
    try:
        ...
        configured = 0
    return max(limit, min(configured or (limit * 5), 100000))


def _max_audit_read_limit() -> int:
    raw = os.environ.get("AZURE_AUDIT_READ_MAX_LIMIT", "")
    try:
        configured = int(raw) if raw else 0
    except ValueError:
        configured = 0
    return max(1000, min(configured or 50000, 100000))
