...
def unsafe_combinations(*, run_probes: bool = True) -> List[str]:
    ...
    if local_research:
        ...
        if (
            not _b("ACCESS_AUDIT_DIR")
            and not _is_true("ALLOW_REPO_LOCAL_OUTPUTS_FOR_LOCAL_RESEARCH")
        ):
            try:
                from app.security.local_paths import credentialed_output_dir

                credentialed_output_dir()
            except Exception:
                problems.append(
                    "LOCAL_CREDENTIALED_RESEARCH=true requires ACCESS_AUDIT_DIR "
                    "or a safe local credentialed output directory outside the repo"
                )
    if azure_demo:
