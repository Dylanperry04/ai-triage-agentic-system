...
This section documents the current full-MIMIC acuity display convention and the
deterministic vital override. Earlier demo-model notes are historical only and
are not live prediction sources. Dylan is the reviewer of record on these
numbers; none are clinically approved.
...
## The full-MIMIC acuity model

The accepted model-selection workflow is
`ml_training/full_mimic/compare_models.py`, run on an approved credentialed
environment against full MIMIC-IV-ED v2.2 from `MIMIC_FULL_ED_DIR`. The model is
not bundled in the repo; serving reads a reviewed artefact from
`MIMIC_FULL_MODEL_PATH`. The deprecated demo/baseline trainers are not live
prediction sources. Features are triage-time ONLY (chiefcomplaint, temperature,
heartrate, resprate, o2sat, sbp, dbp, pain, gender, arrival_transport, and
derived complaint/vital features). Metrics are research evidence only, NOT
clinical validation.

LEAKAGE EXCLUDED: acuity-as-feature; disposition, outtime, hadm_id; the full
vitalsign.csv in-stay time-series; diagnosis.csv; medrecon.csv; and pyxis.csv.
Enforced by feature engineering, model-comparison checks, and leakage tests.
...
- Known limitations: no UHL/prospective/clinician validation has been completed,
  and any full-MIMIC model artefact/metrics must be generated and reviewed on an
  approved credentialed environment before use. Research only.
