...
Two transports, same enforcement path:
  * TWO-SERVICE: set FASTAPI_BASE_URL to the backend URL. Calls go over HTTP to
    the separate FastAPI service.
  * SINGLE-PROCESS: if FASTAPI_BASE_URL is unset, calls use httpx's
...
Identity propagation: in a trusted deployment the Entra principal header
(X-MS-CLIENT-PRINCIPAL) arrives at Streamlit; the client forwards it (and the
trusted-proxy expectation) so the backend resolves the same identity.
"""
...
def _forward_headers(extra: Optional[Dict[str, str]] = None) -> Dict[str, str]:
    ...
    for k, v in incoming.items():
        if k.lower() in ("x-ms-client-principal", "x-ms-client-principal-id",
            ...
            headers[k] = v
    # Forward a local review role only when the backend has explicitly enabled it.
    try:
        import streamlit as st
...
def workflow_queue(
    *,
    ...
    return _request("GET", "/workflow/queue", params=params)


def sweep_overdue_vitals(limit: int = 50000) -> Any:
    return _request("POST", "/workflow/overdue-vitals/sweep", params={"limit": limit})
...
def followup_case(
    ...
    *,
    ...
    scan_uploads: Optional[list[Dict[str, Any]]] = None,
) -> Any:
    return _request(
        "POST",
        f"/cases/{case_uid}/followups",
        json={
            "updated_vitals": updated_vitals,
            "updated_complaint": updated_complaint,
            "updated_context": updated_context,
            "scan_uploads": scan_uploads or [],
        },
    )


def acknowledge_overdue_vitals(case_uid: str) -> Any:
    return _request("POST", f"/cases/{case_uid}/vitals/acknowledge-overdue")


def mark_overdue_vitals_alert(case_uid: str) -> Any:
    return _request("POST", f"/cases/{case_uid}/vitals/mark-overdue-alert")
...
def followup_explain_case(
    ...
    *,
    ...
    return _request(
        "POST",
        f"/cases/{case_uid}/followups/explanations",
        json={
            "updated_vitals": updated_vitals,
            "updated_complaint": updated_complaint,
            "question": question,
...
def audit_dashboard(**filters: Any) -> Any:
    params = {k: v for k, v in filters.items() if v not in (None, "", [])}
    return _request("GET", "/audit/dashboard", params=params)
...
def cost_estimate() -> Any:
    return _request("GET", "/cost/estimate")


def system_assistant(question: str) -> Any:
    return _request("POST", "/system/assistant", json={"question": question})
...
def reads_must_use_backend() -> bool:
    ...
    non-sensitive public sample/test profile.
    """
