...
describe("navForSession — role-scoped navigation matches the backend matrix", () => {
  ...
  it("ITD (security_admin): the only role with the ITD console, plus health", () => {
    const items = nav("security_admin");
    expect(items).toEqual(expect.arrayContaining(["itd", "health", "escalations"]));
    // Security Events was folded into the ITD console: no role gets it as a tab.
    expect(items).not.toContain("security");
  });
