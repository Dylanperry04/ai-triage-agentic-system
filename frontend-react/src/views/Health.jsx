...
export default function Health({ session }) {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      api.health(), api.runtimeStatus(), api.fullMimicStatus(), api.llmStatus(), api.governanceReport(), api.systemMeta(),
      ...
      setD({
        ...
        mimic: m.status === "fulfilled" ? m.value : null,
  ...
  const { health, runtime, mimic, llm, gov, meta, perf } = d;
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: 11 }}>
        ...
        <Light ok={mimic?.full_mimic_loadable || meta?.mimic_full_model_configured} warn={!mimic?.full_mimic_loadable} label="Prediction model artefact" detail={meta?.mimic_full_model_configured ? "Model path configured" : "Not configured — assessments fall back to the rules engine"} />
        ...
        <Light ok={String(meta?.rules_status || "").includes("ACTIVE")} label="Rules engine" detail={String(meta?.rules_status || "").includes("ACTIVE") ? "Provisional MTS research ruleset active — clinician review required" : "No automated categorisation configured"} />
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: 11 }}>
        ...
        <Light ok={meta?.clinical_use === "NOT_FOR_CLINICAL_USE"} label="Research-use labelling" detail="Not for clinical use — displayed across the product and asserted by the backend." />
      ...
      {card ? (
        <Card style={{ padding: 16 }}>
          ...
          <div style={{ fontSize: 12, color: T.slate, marginTop: 11, lineHeight: 1.5 }}>Figures from the training model card generated on the approved environment. Full artefacts (calibration, confusion matrix, subgroup metrics) live under Model Evidence.</div>
        </Card>
