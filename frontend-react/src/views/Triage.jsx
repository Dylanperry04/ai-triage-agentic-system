...
function AdvisoryPanel({ c, assessment, loading, loadingLabel, error, onAccept, onOverride, onEscalate, onRequestInfo, onReassess, onRetry, onExplain, canDecide, canAssess, canExplainAcuity, explainLoading }) {
  if (!c) return null;
  if (loading) return (
    <Card style={{ padding: 16 }}>
      <Eyebrow>Model estimate</Eyebrow>
  ...
  if (error) return (
    <Card style={{ padding: 16 }}>
      <Eyebrow style={{ marginBottom: 10 }}>Model estimate</Eyebrow>
  ...
  const cat = catOf(prio);
  const conf = assessment.top_class_confidence ?? assessment.confidence;
    ...
    <div className="fade-up" key={c.case_uid}>
        ...
        <div style={{ padding: 16 }}>
          ...
          {conf != null && (
            <div style={{ marginTop: 14 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}><Eyebrow>Model confidence</Eyebrow></div>
          ...
          {assessment.override_applied && <div style={{ marginTop: 10, fontSize: 12.5, color: T.ink, background: "#F5F7F7", border: `1px solid ${T.borderSoft}`, borderRadius: 8, padding: "8px 11px", lineHeight: 1.45 }}><b>Rules override:</b> {assessment.override_note || "deterministic vitals rule adjusted the ML proposal."}</div>}
          {probs && (
            <div style={{ marginTop: 14 }}>
      ...
      {canDecide && (
        <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 12 }}>
          <Btn kind="dark" onClick={onAccept} style={{ padding: "12px", fontSize: 14 }} disabled={!prio}><CheckCircle2 size={15} style={{ verticalAlign: -3, marginRight: 7 }} />Accept & log {acuityLabel(prio, "Acuity —")}</Btn>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            <Btn kind="quiet" onClick={onOverride}>Override…</Btn>
            <Btn kind="quiet" onClick={onEscalate}><ArrowUpRight size={14} style={{ verticalAlign: -2, marginRight: 5 }} />Escalate…</Btn>
          </div>
          {canAssess && (
            <Btn kind="quiet" onClick={onReassess} style={awaitingInfo ? { border: "1.5px solid #E86C00", color: "#8A4B00" } : undefined}>
              <Activity size={14} style={{ verticalAlign: -2, marginRight: 6 }} />{awaitingInfo ? "Record requested information" : "Record new observations"}
            </Btn>
          )}
          <Btn kind="quiet" onClick={onRequestInfo}><Undo2 size={14} style={{ verticalAlign: -2, marginRight: 6 }} />Request more information…</Btn>
          {canExplainAcuity && <Btn kind="quiet" onClick={onExplain} disabled={explainLoading}><Sparkles size={14} style={{ verticalAlign: -2, marginRight: 6 }} />{explainLoading ? "Explaining…" : "Explain this acuity"}</Btn>}
        </div>
      )}
    </div>
  );
...
/* Reassessment / requested-information form. Vitals are entered in the units
   clinicians use here (Temp in °C) and converted to the backend's MIMIC units
...
/* Multi-agent case-acuity explanation — sits on the triage-review screen under
   the ML estimate. A team of agents (Intake -> Validation -> Safety review ->
   Explanation) explains WHY the deterministic model/rules produced this acuity.
  ...
  return (
    <Card style={{ marginTop: 12, padding: 16 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
        ...
        <div style={{ fontSize: 13.5, fontWeight: 700 }}>Assessment explanation</div>
      </div>
      <div style={{ fontSize: 11.5, color: T.grey500, lineHeight: 1.45, marginBottom: 12 }}>
        The explanation summarizes why the model and rules produced this acuity. It explains the estimate — it does not decide it.
      </div>

      {state === "idle" && (
        <Btn kind="quiet" style={{ width: "100%" }} onClick={run}><Sparkles size={14} style={{ verticalAlign: -2, marginRight: 6 }} />Explain this acuity</Btn>
      )}
...
export default function Triage({ cases, casesError, refresh, selectedUid, setSelectedUid, query, searchActive, searchBusy, toast, canDecide, canAssess, canExplainAcuity, presentation, serverTotal, hasMore, onLoadMore, loadingMore }) {
  ...
  const [showExplainTurns, setShowExplainTurns] = useState(false);
  ...
  useEffect(() => { setExplainState("idle"); setExplainResult(null); setExplainErr(null); setShowExplainTurns(false); }, [sel?.case_uid]);
  ...
  const doDecision = async (fn, okTitle, okBody) => {
    if (busyRef.current) return; busyRef.current = true;
    try { await fn(); toast(okTitle, okBody); setModal(null); setReason(""); advance(); refresh(); }
  ...
  const runExplanation = () => {
    if (!sel || explainState === "loading") return;
    setExplainState("loading");
    setExplainResult(null);
    setExplainErr(null);
    setShowExplainTurns(false);
    api.multiagentExplainCase(sel.case_uid)
      .then((r) => {
        setExplainResult(r);
        setExplainState("done");
        setModal("explain");
        if (r.status === "SAFETY_FAIL") toast("Explanation withheld", "The safety filter blocked the generated explanation.", "err");
      })
      .catch((e) => {
        setExplainErr(e.detail?.reason || e.detail || e.message);
    ...
    <div style={{ display: "flex", gap: 16, height: "100%", minHeight: 0 }}>
      ...
      <div style={{ width: 330, flexShrink: 0, overflowY: "auto", paddingRight: 2 }}>
        <AdvisoryPanel c={sel} assessment={a} loading={selectedAssessLoading} loadingLabel={selectedLoadingLabel} error={selectedAssessError}
          canDecide={canDecide} canAssess={canAssess} canExplainAcuity={canExplainAcuity} explainLoading={explainState === "loading"}
          onAccept={() => doDecision(() => decisions.accept(sel.case_uid, { systemPrediction: sysPred }), `${acuityLabel(priorityFromCategory(sysPred), "Acuity —")} logged`, `${patientWithEncounter(sel)} moved to the review queue.`)}
      ...
      {modal === "explain" && sel && (
        <Modal title={`Acuity explanation — ${patientWithEncounter(sel)}`} onClose={() => setModal(null)} width={620}>
          {explainState === "error" && <ErrorNote>Explanation unavailable: {String(explainErr)}</ErrorNote>}
          {explainResult?.status === "NOT_CONFIGURED" && (
            <div style={{ fontSize: 13, color: T.slate, lineHeight: 1.55 }}>
              The explanation service is not configured in this environment.
            </div>
          )}
          {explainResult?.status === "PASS" && (
            <>
              <div style={{ fontSize: 13.5, color: T.ink, lineHeight: 1.6 }}>{explainResult.final_explanation}</div>
              {(explainResult.agent_turns || []).length > 0 && (
                <>
                  <Btn kind="ghost" style={{ marginTop: 12, paddingLeft: 0 }} onClick={() => setShowExplainTurns((v) => !v)}>
                    <ChevronDown size={13} style={{ transform: showExplainTurns ? "rotate(180deg)" : "none", transition: "transform .15s", verticalAlign: -2, marginRight: 5 }} />
                    {showExplainTurns ? "Hide" : "Show"} agent steps
                  </Btn>
                  {showExplainTurns && (
                    <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 8 }}>
                      {explainResult.agent_turns.map((turn, i) => {
                        const meta = AGENT_META[turn.agent] || { icon: Sparkles, label: turn.agent, blurb: "" };
                        const Icon = meta.icon;
                        return (
                          <div key={i} style={{ display: "flex", gap: 9, alignItems: "flex-start" }}>
                            <div style={{ flexShrink: 0, width: 26, height: 26, borderRadius: 7, background: T.green50, display: "flex", alignItems: "center", justifyContent: "center", marginTop: 1 }}><Icon size={14} style={{ color: T.green700 }} /></div>
                            <div>
                              <div style={{ fontSize: 11.5, fontWeight: 700, color: T.slate }}>{meta.label}</div>
                              <div style={{ fontSize: 12.5, color: T.ink, lineHeight: 1.5, marginTop: 2 }}>{turn.text}</div>
                            </div>
                          </div>
                        );
          ...
          {explainResult?.status === "SAFETY_FAIL" && (
            <ErrorNote>The generated explanation did not pass the safety filter and was withheld.</ErrorNote>
          )}
          <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 16 }}>
            <Btn kind="quiet" onClick={() => setModal(null)}>Close</Btn>
          </div>
        </Modal>
      )}
