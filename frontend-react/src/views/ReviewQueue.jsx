...
export default function ReviewQueue({ cases, casesError, refresh, decisionMap, toast, session, escalationsOnly = false, focusUid = null, onFocusHandled }) {
  ...
  const reloadRows = useCallback(() => {
    ...
    /* Reviewed cases must stay visible until discharged or closed. This view has
       its own fetch, separate from the notification poll in App.jsx: raising
       only that one left the queue itself capped, so cases past the 500th
       oldest active entry silently vanished from the worklist. */
    api.workflowQueue({ limit: 5000 })
  ...
  /* Card colour comes from the acuity the backend PERSISTED with the decision
     ...
     not landed yet — it is never the source of truth, so colours survive a
     reload and stay until the case is closed. */
  const prioOf = (c) => {
    const persisted = priorityFromCategory(
      c.workflow_state?.assigned_acuity ?? c.assigned_acuity, null,
    );
    return persisted ?? decisionMap[c.case_uid] ?? null;
  };
