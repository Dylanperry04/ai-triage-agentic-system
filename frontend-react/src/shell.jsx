...
import { Bell, LogOut, Search, ChevronLeft, ChevronRight, Activity, ClipboardList, LayoutDashboard, ShieldCheck, ScrollText, ArrowUpRight, HeartPulse, ServerCog, MessageSquareWarning, LockKeyhole, BarChart3 } from "lucide-react";
...
export const NAV = {
  ...
  security:    { label: "Security Events",  icon: LockKeyhole, group: "System" },
...
export function navForSession(session) {
  ...
  if (perms.has("can_view_security_status")) items.push("security");
  if (tabs.has("governance") || tabs.has("system_status") || roles.has("security_admin")) items.push("health");
  return items;
}

export function Sidebar({ session, tab, setTab, collapsed, setCollapsed, sourceLabel, presentation }) {
  ...
  return (
    <aside style={{ width: collapsed ? 72 : 252, transition: "width .22s ease", background: `linear-gradient(180deg, ${T.green900}, #013A33)`, color: "#E9F5F1", display: "flex", flexDirection: "column", flexShrink: 0, overflow: "hidden" }}>
      ...
      <div style={{ padding: 10, borderTop: "1px solid rgba(255,255,255,0.09)" }}>
        {!collapsed && !presentation && <div style={{ fontSize: 10.5, color: "#7FB5A8", padding: "2px 10px 8px", lineHeight: 1.5 }}>{sourceLabel || "Research prototype"} · not for clinical use</div>}
