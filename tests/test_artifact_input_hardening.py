...
class RawFrameAcuityModel:
    classes_ = np.array([1, 2, 3, 4, 5])

    def predict(self, X):
        assert list(X.columns) == list(RAW_TRIAGE_INPUT_COLUMNS)
        return np.array([2 for _ in range(len(X))])

    def predict_proba(self, X):
        assert list(X.columns) == list(RAW_TRIAGE_INPUT_COLUMNS)
        return np.array([[0.05, 0.70, 0.20, 0.04, 0.01] for _ in range(len(X))])
...
class TestModelLoadHardening:
    ...
    def test_v2_raw_dataframe_artifact_predicts(self, tmp_path, monkeypatch):
        art = Path(tmp_path) / "raw-v2.joblib"
        joblib.dump({
            "model": RawFrameAcuityModel(),
        ...
        r = _run_full_mimic_prediction(_ti())
        assert r.prediction_available is True
        assert r.model_name == "mimic_full_acuity_model_v2_raw_tfidf"
        assert r.predicted_mimic_acuity == 2
        assert r.high_acuity_research_estimate == pytest.approx(0.75)
        assert r.mimic_acuity_probabilities["2"] == pytest.approx(0.70)

    def test_v2_raw_dataframe_schema_mismatch_fails_closed(self, tmp_path, monkeypatch):
        art = Path(tmp_path) / "bad-raw-v2.joblib"
        joblib.dump({
            "model": RawFrameAcuityModel(),
            "artifact_contract_version": RAW_TRIAGE_ARTIFACT_CONTRACT_VERSION,
            "input_type": RAW_TRIAGE_INPUT_TYPE,
            "raw_input_columns": ["chiefcomplaint"],
            "sklearn_version": sklearn.__version__,
        }, art)
        monkeypatch.setenv("MIMIC_FULL_MODEL_PATH", str(art))
        monkeypatch.delenv("MIMIC_FULL_MODEL_SHA256", raising=False)
...
class TestReviewFieldValidation:
    ...
    def test_comment_is_redacted(self):
        ...
        assert "John Smith" in r.review_comment
        assert "[REDACTED_NAME]" not in r.review_comment
