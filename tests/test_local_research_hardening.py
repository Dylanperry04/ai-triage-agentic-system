...
class TestLocalResearchAuditFailClosed:
    ...
    def test_local_credentialed_output_is_project_scoped(self, monkeypatch, tmp_path):
        from pathlib import Path

        from app.security.local_paths import credentialed_artifact_path, project_root
        from app.storage.case_state_repository import read_case_states

        monkeypatch.delenv("PATIENT_DATA_MODE", raising=False)
        monkeypatch.delenv("ACCESS_AUDIT_DIR", raising=False)
        monkeypatch.delenv("LOCAL_CREDENTIALED_OUTPUT_DIR_EXACT", raising=False)
        monkeypatch.delenv("LOCAL_CREDENTIALED_OUTPUT_SCOPE", raising=False)
        monkeypatch.setenv("LOCAL_CREDENTIALED_RESEARCH", "true")
        monkeypatch.setenv("BACKEND_BIND_HOST", "127.0.0.1")

        shared = tmp_path / "shared-output"
        shared.mkdir()
        # Legacy unscoped state from an older run must not seed a fresh package.
        (shared / "case_workflow_state.jsonl").write_text(
            '{"case_uid":"old-case","case_status":"accepted"}\n',
            encoding="utf-8",
        )
        monkeypatch.setenv("LOCAL_CREDENTIALED_OUTPUT_DIR", str(shared))

        default_path = Path("data/processed/case_workflow_state.jsonl")
        resolved = credentialed_artifact_path(default_path, purpose="workflow-state test")

        assert resolved.parent == shared / project_root().name
        assert read_case_states(default_path) == []
