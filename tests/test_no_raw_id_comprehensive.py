...
class TestNoRawIdAcrossEndpoints:
    ...
    def test_followup_multiagent_explanation_no_raw_ids(self, monkeypatch):
        ...
        cuid = client.get("/cases",
                          headers=self._h(["security-admins"])).json()["cases"][0]["case_uid"]
        r = client.post(
            f"/cases/{cuid}/followups/multiagent-explanations",
            headers=self._h(["security-admins"]),
    ...
    def test_followup_single_agent_explanation_no_raw_ids(self, monkeypatch):
        monkeypatch.setenv("ALLOW_PATIENT_EXPLANATION_DEBUG_ROUTE", "true")
        def _fake_llm(evidence, clinician_question=None):
            ...
            return ExplanationResult(
                ...
                explanation_text="Follow-up single-agent answer only.",
            )
