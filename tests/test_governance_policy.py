...
from app.governance.wandb_logging import (
    log_governance_run,
    wandb_available,
    wandb_configured,
    wandb_sdk_status,
...
def test_governance_report_endpoint_returns_required_shape(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("REQUIRE_CLINICIAN_APPROVED_RULESET", raising=False)
    monkeypatch.delenv("CLAIM_OFFICIAL_MANCHESTER_TRIAGE", raising=False)
    ...
    assert "ml model artifact" in issues
    assert "manchester triage ruleset" not in issues
    assert payload["approval_boundaries"]["system_level_ml_model_approval"] == "not_approved"


def test_governance_report_blocks_on_manchester_only_when_ruleset_mode_claimed(monkeypatch, tmp_path):
    monkeypatch.delenv("PATIENT_DATA_MODE", raising=False)
    monkeypatch.delenv("LOCAL_CREDENTIALED_RESEARCH", raising=False)
    ...
    payload = r.json()
    issues = " ".join(payload["blocking_issues"]).lower()
    assert "manchester/ruleset-driven" in issues
    assert (
        payload["approval_boundaries"]["official_manchester_ruleset_approval"]
        == "required_but_not_configured"
    )


def test_governance_report_uses_model_report_dir_fallback(monkeypatch, tmp_path):
    monkeypatch.delenv("PATIENT_DATA_MODE", raising=False)
    monkeypatch.delenv("LOCAL_CREDENTIALED_RESEARCH", raising=False)
    monkeypatch.delenv("MIMIC_FULL_REPORT_DIR", raising=False)
    monkeypatch.delenv("MIMIC_FULL_OUTPUT_DIR", raising=False)
    report_dir = tmp_path / "model_reports"
    report_dir.mkdir()
    for filename in (
        "full_mimic_schema_report.json",
    ...
    assert controls["full_mimic_model_comparison"]["status"] == "PASS"
    assert controls["full_mimic_model_card"]["status"] == "PASS"
    assert controls["full_mimic_dataset_card"]["status"] == "PASS"
...
def test_governance_wandb_logging_goes_through_backend(monkeypatch):
    ...
    assert "reason" in status.json()
    assert status.json()["optional"] is True
    assert status.json()["required_for_runtime_governance"] is False
    payload = {
...
def test_invalid_wandb_module_is_not_reported_available(monkeypatch):
    fake_wandb = types.SimpleNamespace(__file__="C:/repo/wandb.py")
    monkeypatch.setitem(sys.modules, "wandb", fake_wandb)

    status = wandb_sdk_status()
    assert status["available"] is False
    assert "missing init" in status["reason"]
    assert wandb_available() is False

    res = log_governance_run(
        "triage-governance-test",
        {"overall_status": "PASS", "passed": 0, "total": 0, "checks": []},
        mode="offline",
    )
    assert res["status"] == "SKIPPED"
    assert "not the W&B SDK" in res["reason"]
