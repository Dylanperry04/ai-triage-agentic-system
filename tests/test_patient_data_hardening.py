...
class TestAuditSinkHardening:
    ...
    def test_case_workflow_state_reads_from_durable_sink_in_patient_mode(self, monkeypatch, tmp_path):
        import app.security.audit_sink as sinks
        from app.storage.case_state_repository import latest_case_state, read_case_states

        history_records = [
            {
                "record_kind": "case_workflow_state",
                "case_uid": "case-a",
            ...
            },
            {
                "record_kind": "case_workflow_state",
                "case_uid": "case-a",
                "updated_at_utc": "2026-07-20T11:00:00+00:00",
        ...
        class FakeDurable:
            def read_recent(self, limit, record_kind=None, since_utc=None):
                if record_kind == "case_workflow_state_current":
                    return current_records[-limit:]
                assert record_kind == "case_workflow_state"
        ...
        monkeypatch.setattr(sinks, "get_audit_sink", lambda *a, **k: FakeDurable())

        path = tmp_path / "case_workflow_state.jsonl"
        assert read_case_states(path) == [
            {
                "record_kind": "case_workflow_state",
                "case_uid": "case-a",
                "updated_at_utc": "2026-07-20T11:00:00+00:00",
        ...
        assert latest_case_state(path, "case-a")["case_status"] == "discharged"

    def test_current_workflow_state_point_read_failure_fails_closed(self, monkeypatch, tmp_path):
        import app.security.audit_sink as sinks
        from app.storage.case_state_repository import read_current_case_state
        from app.storage.guarded_writer import PatientDataStorageError

        class FakeClient:
            def get_entity(self, partition_key, row_key):
                raise TimeoutError("durable store unavailable")

        class FakeDurable:
            client = FakeClient()

            def read_recent(self, limit, record_kind=None, since_utc=None):
                pytest.fail("fallback scan must not hide point-read outages")

        monkeypatch.setenv("PATIENT_DATA_MODE", "true")
        monkeypatch.setenv("AUDIT_SINK", "durable")
        monkeypatch.setattr(sinks, "get_audit_sink", lambda *a, **k: FakeDurable())

        with pytest.raises(PatientDataStorageError, match="point read failed"):
            read_current_case_state(tmp_path / "case_workflow_state.jsonl", "case-a")

    def test_append_case_state_writes_current_before_history(self, monkeypatch, tmp_path):
        from app.storage.case_state_repository import append_case_state
        import app.storage.guarded_writer as guarded_writer

        calls = []

        def fake_write_record(path, record, *, record_kind):
            calls.append(record_kind)
            if record_kind == "case_workflow_state":
                raise RuntimeError("history write failed")

        monkeypatch.setattr(guarded_writer, "write_record", fake_write_record)

        with pytest.raises(RuntimeError, match="history write failed"):
            append_case_state(tmp_path / "case_workflow_state.jsonl", {
                "case_uid": "case-a",
                "case_status": "discharged",
            })

        assert calls == ["case_workflow_state_current", "case_workflow_state"]
