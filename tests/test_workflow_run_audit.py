...
def test_read_workflow_runs_skips_malformed_legacy_jsonl_lines():
    with tempfile.TemporaryDirectory() as d:
        path = Path(d) / "workflow_runs.jsonl"
        wf = run_workflow(_synthetic_mimic_case())
        rec = build_workflow_run_record(wf, "rid-good", "2026-06-18T00:00:00Z")
        path.write_bytes(
            b'{"workflow_run_id":"truncated"\n'
            + orjson.dumps(rec.model_dump(mode="json"))
            + b"\n"
            + b'n"}\n'
        )
        ...
        assert len(out) == 1
        assert out[0].workflow_run_id == "rid-good"
