...
def test_label_remap_classifier_routes_sample_weight_through_pipeline():
    import numpy as np
    ...
    class RecordingEstimator(BaseEstimator, ClassifierMixin):
        def fit(self, X, y, sample_weight=None):
            ...
            self.y_seen_ = np.asarray(y)
            self.sample_weight_seen_ = np.asarray(sample_weight)
            return self
    ...
    weights = np.array([3.0, 2.0, 2.0, 1.0, 1.0, 1.0])
    wrapped = LabelRemapClassifier(Pipeline([
        ("identity", FunctionTransformer(validate=False)),
        ("estimator", RecordingEstimator()),
    ]))

    wrapped.fit(X, y, sample_weight=weights)

    recorder = wrapped.estimator_.named_steps["estimator"]
    assert wrapped.classes_.tolist() == [1, 2, 3]
    assert recorder.y_seen_.tolist() == [0, 1, 1, 2, 2, 2]
    assert recorder.sample_weight_seen_.tolist() == weights.tolist()
...
class TestCompareModelsSafetyFirst:
    ...
    def test_raw_text_quick_mode_defaults_to_multiclass_solver(self, monkeypatch):
        from ml_training.full_mimic import compare_models

        monkeypatch.setenv("MIMIC_COMPARE_QUICK", "1")
        monkeypatch.delenv("MIMIC_TFIDF_LOGISTIC_SOLVER", raising=False)
        cands = compare_models._raw_triage_candidates(["raw_tfidf_word_char_logistic"])

        assert len(cands) == 1
        _, pipe = cands[0]
        assert pipe.named_steps["estimator"].solver == "saga"
    ...
    def test_compare_models_runs_and_selects(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(_mfl, "load_mimic_full_cases_triage_time", lambda: cases)
        ...
        for c in comp["candidates"]:
            ...
            assert "decision_rule_tuning" in c
            assert c["imbalance_strategy"] in {
                "none",
                "class_weight_balanced",
                "balanced_sample_weight",
                "smote_training_only_after_split",
            }
            assert c["balance_before_split"] is False
            assert c["validation_and_test_resampled"] is False
            assert "train_distribution_before" in c
            assert "validation_distribution" in c
            assert "test_distribution" in c
            assert c["decision_rule_tuning"]["test_set_used"] is False
        ...
        assert (out / "full_mimic_threshold_tuning_report.json").exists()
        assert (out / "full_mimic_selected_decision_rule.json").exists()
        assert (out / "full_mimic_high_acuity_detector_report.json").exists()
        assert (out / "full_mimic_class_distribution.json").exists()
        assert (out / "full_mimic_class_distribution.csv").exists()
        assert (out / "all_models_roc_auc_comparison.csv").exists()
        assert (out / "selected_model_binary_curve_report.json").exists()
        assert (out / "selected_model_roc_curve.csv").exists()
        assert (out / "selected_model_pr_curve.csv").exists()
        # corrected evaluation methodology is reflected in the report
        assert comp["split_kind"] == "stratified_patient_grouped"
        assert comp["class_distribution_report"] == "full_mimic_class_distribution.json"
        assert comp["class_distribution"]["balance_before_split"] is False
        assert comp["class_distribution"]["validation_and_test_resampled"] is False
        assert comp["test_set_used_for_decision_rule_tuning"] is False
        assert comp["decision_rule_tuned_on"] == "validation"
        assert comp["selected_decision_rule"]["type"] in {
            "argmax", "high_acuity_threshold", "ordinal_cost_sensitive",
        }
        assert comp["binary_high_acuity_detector_report"][
            "test_set_used_for_threshold_selection"
        ] is False
        assert "untouched_test_metrics" in comp
        utm = comp["untouched_test_metrics"]
        assert "decision_rule_applied" in utm
        assert "auroc_pr_auc" in utm and "accuracy_95ci" in utm
        assert "over_triage_specificity" in utm and "subgroups_by_sex" in utm
        assert "high_acuity_precision" in utm["over_triage_specificity"]
        ...
        assert provenance["split_type"] == "stratified_patient_level_group_split"
        assert provenance["final_test_evaluation_once"] is True
        assert provenance["test_set_used_for_decision_rule_tuning"] is False
        assert provenance["class_distribution_report"] == "full_mimic_class_distribution.json"
        dist = json.loads((out / "full_mimic_class_distribution.json").read_text())
        assert dist["balance_before_split"] is False
        assert dist["validation_and_test_resampled"] is False
        assert set(dist) >= {"overall", "train", "validation", "test"}
        curve = json.loads((out / "selected_model_binary_curve_report.json").read_text())
        assert curve["binary_target"] == "acuity_1_2_high_acuity_vs_3_5_non_high_acuity"
        assert curve["test_set_used_for_model_selection"] is False
    ...
    def test_compare_models_can_write_serving_eligible_raw_text_artifact(
        ...
        monkeypatch.setenv("PATIENT_DATA_MODE", "true")
        monkeypatch.setenv("MIMIC_FULL_OUTPUT_DIR", str(out))
        cases = self._synthetic_cases(n=160)
        for case in cases:
            triage = case["triage"]
            if triage["acuity"] in (1, 2):
                triage["chiefcomplaint"] = "SOB AMS chest pain"
                case["edstay"]["arrival_transport"] = "AMBULANCE"
            else:
                triage["chiefcomplaint"] = "wound eval lower back pain"
                case["edstay"]["arrival_transport"] = "WALK IN"
        import app.data_pipeline.mimic_full_loader as _mfl
        monkeypatch.setattr(_mfl, "load_mimic_full_cases_triage_time", lambda: cases)
        ...
        assert rc == 0
        comp = json.loads((out / "full_mimic_model_comparison.json").read_text())
        assert comp["selected_model"] == "raw_tfidf_word_char_logistic"
        assert comp["selected_artifact_contract_version"] == RAW_TRIAGE_ARTIFACT_CONTRACT_VERSION
        assert comp["selected_input_type"] == RAW_TRIAGE_INPUT_TYPE
        assert "subject_id" not in json.dumps(comp)
        assert "stay_id" not in json.dumps(comp)

        bundle = joblib.load(out / "mimic_full_acuity_selected.joblib")
        assert bundle["artifact_contract_version"] == RAW_TRIAGE_ARTIFACT_CONTRACT_VERSION
        assert bundle["input_type"] == RAW_TRIAGE_INPUT_TYPE
        assert bundle["raw_input_columns"] == list(RAW_TRIAGE_INPUT_COLUMNS)
        assert bundle["decision_rule_tuned_on"] == "validation"
        assert bundle["test_set_used_for_decision_rule_tuning"] is False
        assert bundle["decision_rule"]["type"] in {
            "argmax", "high_acuity_threshold", "ordinal_cost_sensitive",
        }
        assert "feature_names" not in bundle

    def test_decision_rule_tuning_uses_validation_only(self):
        import numpy as np
        from ml_training.full_mimic import compare_models
        ...
        class ProbModel:
            ...
            def predict(self, X):
                return np.array([3, 3, 3, 4])

            def predict_proba(self, X):
                return np.array([
                    [0.10, 0.55, 0.25, 0.10, 0.00],
                    [0.08, 0.50, 0.30, 0.12, 0.00],
                    [0.01, 0.05, 0.70, 0.24, 0.00],
                    [0.00, 0.02, 0.18, 0.70, 0.10],
                ])

        labels = [1, 2, 3, 4, 5]
        yva = np.array([2, 2, 3, 4])
        model = ProbModel()
        base = compare_models._metrics_from_predictions(
            "demo", yva, model.predict(None), labels,
            proba_aligned=model.predict_proba(None))

        tuning = compare_models._tune_candidate_decision_rule(
            "demo", model, None, yva, labels, base,
            compare_models._selection_constraints(profile="balanced_safety"))

        assert tuning["validation_only"] is True
        assert tuning["test_set_used"] is False
        assert tuning["selected_rule"]["type"] in {
            "argmax", "high_acuity_threshold", "ordinal_cost_sensitive",
        }

    def test_balanced_safety_default_recall_floor_matches_final_scripts(self, monkeypatch):
        from ml_training.full_mimic import compare_models

        monkeypatch.delenv("MIMIC_MIN_HIGH_ACUITY_RECALL", raising=False)
        constraints = compare_models._selection_constraints(profile="balanced_safety")
        assert constraints["min_high_acuity_recall"] == 0.80

    def test_balanced_profile_rejects_predict_everything_urgent_threshold(self):
        import numpy as np
        from ml_training.full_mimic import compare_models

        class AggressiveProbModel:
            classes_ = np.array([1, 2, 3, 4, 5])
            ...
            def predict_proba(self, X):
                return np.array([
                    [0.05, 0.55, 0.35, 0.05, 0.00],
                    [0.04, 0.58, 0.33, 0.05, 0.00],
                    [0.00, 0.06, 0.74, 0.20, 0.00],
                    [0.00, 0.06, 0.64, 0.30, 0.00],
                    [0.00, 0.06, 0.54, 0.40, 0.00],
                ])

        labels = [1, 2, 3, 4, 5]
        yva = np.array([2, 2, 3, 3, 4])
        model = AggressiveProbModel()
        base = compare_models._metrics_from_predictions(
            "demo", yva, model.predict(None), labels,
            proba_aligned=model.predict_proba(None))

        constraints = compare_models._selection_constraints(
            profile="balanced_safety",
            min_specificity=0.50,
            max_predicted_urgent_rate=0.50,
            min_high_acuity_recall=0.90,
        )
        tuning = compare_models._tune_candidate_decision_rule(
            "demo", model, None, yva, labels, base, constraints)

        threshold_rows = tuning["threshold_curve"]
        threshold_005 = next(row for row in threshold_rows if row["threshold"] == 0.05)

        assert threshold_005["predicted_urgent_rate"] == 1.0
        assert threshold_005["passes_over_triage_constraint"] is False
        if tuning["selected_rule"]["type"] == "high_acuity_threshold":
            assert tuning["selected_rule"]["threshold"] > 0.05
