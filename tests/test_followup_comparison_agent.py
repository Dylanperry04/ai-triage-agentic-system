...
class TestDatasetSpecificFollowUp:
    """#10: MIMIC compares final acuity/priority movement; KTAS compares KTAS class."""

    def _find_mimic_with_acuity(self, target_min=None, target_max=None):
        from app.config import settings
        from app.data_pipeline.mimic_adapter import load_mimic_demo_cases
        from app.agents.orchestrator import run_workflow
        mimic, _ = load_mimic_demo_cases(settings.raw_demo_dir, n=200)
        for c in mimic:
            a = run_workflow(c).final_acuity_assessment.final_acuity
            if a is None:
                continue
            if target_min is not None and a < target_min:
                continue
            if target_max is not None and a > target_max:
                continue
            return run_workflow(c)
        return None

    def test_mimic_acuity_movement_detected(self):
        from app.agents.followup_comparison_agent import compare_follow_up
        from app.schemas.followup import FollowUpLinkRequest
        low = self._find_mimic_with_acuity(target_min=3)   # less urgent (3+)
        high = self._find_mimic_with_acuity(target_max=2)  # more urgent (<=2)
        assert low is not None and high is not None
        link = FollowUpLinkRequest(previous_stay_id=low.stay_id, new_stay_id=high.stay_id,
                                   linked_by="t", link_reason="r", is_demonstration_scenario=True)
        # low -> high acuity (number decreases) = escalation
        r = compare_follow_up(link, low, high)
        assert r.comparison_dataset == "MIMIC-IV-ED-Demo-v2.2"
        assert r.category_movement == "ESCALATION"
        assert r.escalation_detected is True
        # reverse = de-escalation
        link2 = FollowUpLinkRequest(previous_stay_id=high.stay_id, new_stay_id=low.stay_id,
                                    linked_by="t", link_reason="r", is_demonstration_scenario=True)
        r2 = compare_follow_up(link2, high, low)
        assert r2.category_movement == "DE_ESCALATION"

    def test_ktas_follow_up_uses_class_not_mts(self):
        from app.config import settings
        from app.data_pipeline.ktas_adapter import load_ktas_cases
        from app.agents.orchestrator import run_workflow
        from app.agents.followup_comparison_agent import compare_follow_up
        from app.schemas.followup import FollowUpLinkRequest
        ktas, _ = load_ktas_cases(settings.raw_ktas_csv, n=20)
        a, b = run_workflow(ktas[0]), run_workflow(ktas[1])
        link = FollowUpLinkRequest(previous_stay_id=ktas[0].stay_id, new_stay_id=ktas[1].stay_id,
                                   linked_by="t", link_reason="r", is_demonstration_scenario=True)
        r = compare_follow_up(link, a, b)
        assert r.comparison_dataset == "Kaggle-KTAS"
        assert r.previous_ktas_class is not None and r.new_ktas_class is not None
        # KTAS comparison must NOT populate MIMIC acuity fields
        assert r.previous_final_acuity is None and r.new_final_acuity is None
