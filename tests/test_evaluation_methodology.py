...
from ml_training.full_mimic.evaluation import (
    patient_grouped_split, stratified_patient_grouped_split, temporal_split, assert_no_subject_overlap,
...
class TestStratifiedPatientGroupedSplit:
    def test_no_subject_spans_splits(self):
        subject_ids = []
        labels = []
        for p in range(80):
            label = 1 if p < 8 else 2 if p < 32 else 3 if p < 70 else 4
            visits = 2 if p % 7 == 0 else 1
            subject_ids.extend([p] * visits)
            labels.extend([label] * visits)
        tr, va, te = stratified_patient_grouped_split(subject_ids, labels, seed=3)
        assert_no_subject_overlap(subject_ids, tr, va, te)
        assert len(tr) and len(va) and len(te)
        assert len(tr) + len(va) + len(te) == len(subject_ids)

    def test_approximately_preserves_high_acuity_mix(self):
        subject_ids = []
        labels = []
        for p in range(100):
            label = 1 if p < 20 else 3
            subject_ids.append(p)
            labels.append(label)
        tr, va, te = stratified_patient_grouped_split(subject_ids, labels, seed=9)
        arr = np.array(labels)
        for idx in (tr, va, te):
            frac_high = float(np.mean(arr[idx] <= 2))
            assert 0.10 <= frac_high <= 0.30
