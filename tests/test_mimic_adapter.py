...
class TestLoadMimicTable:
    def test_accepts_uncompressed_csv_from_local_extraction(self, tmp_path):
        (tmp_path / "triage.csv").write_text(
            "subject_id,stay_id,temperature,heartrate,resprate,o2sat,sbp,dbp,pain,acuity,chiefcomplaint\n"
            "1,2,98.6,80,18,99,120,80,3,2,CHEST PAIN\n",
            encoding="utf-8",
        )
        df = load_mimic_table(tmp_path, "triage")
        assert len(df) == 1
        assert int(df.iloc[0]["stay_id"]) == 2
        assert df.iloc[0]["chiefcomplaint"] == "CHEST PAIN"

    def test_accepts_nested_csv_directory_from_local_extraction(self, tmp_path):
        nested = tmp_path / "triage.csv"
        nested.mkdir()
        (nested / "triage.csv").write_text(
            "subject_id,stay_id,temperature,heartrate,resprate,o2sat,sbp,dbp,pain,acuity,chiefcomplaint\n"
            "1,3,98.6,82,18,99,120,80,4,2,ABDOMINAL PAIN\n",
            encoding="utf-8",
        )
        df = load_mimic_table(tmp_path, "triage")
        assert len(df) == 1
        assert int(df.iloc[0]["stay_id"]) == 3
        assert df.iloc[0]["chiefcomplaint"] == "ABDOMINAL PAIN"
...
class TestDataframeToCases:
    ...
    def test_source_dataset_label_is_set_correctly(
        ...
        for case in cases:
            ...
            assert case.source_dataset == "MIMIC-IV-ED-Full-v2.2"
    ...
    def test_real_unmodified_orchestrator_runs_on_mimic_case_without_special_casing(
        ...
        assert result.triage_input.source_dataset == "MIMIC-IV-ED-Full-v2.2"
