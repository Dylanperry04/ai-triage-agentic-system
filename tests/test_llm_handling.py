...
Priority-5 LLM-handling proofs:
  ...
  - stronger free-text redaction (phones/dates/SSN), clinical text preserved
"""
import pytest
...
class TestPromptRedaction:
    ...
    def test_free_text_redacted_in_prompt(self):
        ...
        assert "[REDACTED_NAME]" not in prompt
...
class TestQuestionPassedToPrompt:
    def test_screened_question_in_prompt(self):
        ...
        assert "Do not recap all patient facts" in prompt
...
class TestStructuredOutputValidation:
    ...
    def test_direct_answer_format_passes_structure(self):
...
class TestStrongerRedaction:
    ...
    def test_name_pair_not_redacted(self):
        assert redact_text("patient John Smith arrived") == "patient John Smith arrived"
