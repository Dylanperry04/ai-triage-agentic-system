...
class TestRbacAcrossRoutes:
    ...
    def test_governance_auditor_can_view_governance(self, monkeypatch):
        monkeypatch.setenv("PATIENT_DATA_MODE", "true")
        monkeypatch.setenv("TRUSTED_AUTH_PROXY", "true")
        r = client.get("/governance/report", headers=self._h(["governance-auditors"]))
        assert r.status_code in (200, 404, 500)
        assert r.status_code != 403
    ...
    def test_patient_audit_dashboard_reads_durable_sink(self, monkeypatch):
        from app.security import audit_sink as _as
            ...
            "human_review": [
                {
                    ...
                    "created_at_utc": "2026-07-20T10:10:00+00:00",
                    "case_uid": "case-a",
            ...
            "case_workflow_state": [
                {
                    ...
                    "updated_at_utc": "2026-07-20T10:11:00+00:00",
                    "case_uid": "case-a",
                    "source_dataset": "MIMIC-IV-ED-Full-v2.2",
        ...
        class _ReadableDurable:
            ...
            def read_recent(self, limit, record_kind=None, since_utc=None):
                return list(records.get(record_kind, []))[-limit:]

        monkeypatch.setattr(_as, "get_audit_sink", lambda *a, **k: _ReadableDurable())
        monkeypatch.setenv("PATIENT_DATA_MODE", "true")
        monkeypatch.setenv("TRUSTED_AUTH_PROXY", "true")
        r = client.get("/audit/dashboard", headers=self._h(["security-admins"]))
        assert r.status_code == 200
        payload = r.json()
        assert payload["source"] == "durable"
        assert payload["aggregations"]["summary"]["open_escalations"] == 1
...
class TestDemoModeStillEnforcesRbac:
    ...
    def test_auth_session_reports_backend_permissions(self, monkeypatch):
        ...
        assert "maintainability" in payload["visible_tabs"]
        assert "itd_ask_tools" not in payload["visible_tabs"]
        assert "followup_comparison" not in payload["visible_tabs"]
    ...
    def test_security_admin_is_displayed_as_itd_with_all_tabs(self, monkeypatch):
        for v in [
            ...
            monkeypatch.delenv(v, raising=False)
        r = client.get("/auth/session", headers={"X-Demo-Role": "security_admin"})
        assert r.status_code == 200
        payload = r.json()
        assert payload["roles"] == ["security_admin"]
        assert payload["display_roles"] == ["ITD"]
        assert "can_ask_chatbot" in payload["permissions"]
        assert "itd_ask_tools" in payload["visible_tabs"]
        assert "governance" in payload["visible_tabs"]
        assert "cost_runtime" not in payload["visible_tabs"]
        assert "followup_comparison" not in payload["visible_tabs"]
        assert "system_status" not in payload["visible_tabs"]

    def test_itd_system_assistant_is_admin_only_and_refuses_patient_questions(self, monkeypatch):
        for v in [
            "PATIENT_DATA_MODE",
            "AUTH_REQUIRED",
            "TRUSTED_AUTH_PROXY",
        ...
        denied = client.post(
            ...
            json={"question": "What is the model status?"},
        )
        assert denied.status_code == 403

        patient_question = client.post(
            "/system/assistant",
            headers={"X-Demo-Role": "security_admin"},
            json={"question": "What acuity should this patient get?"},
        )
        assert patient_question.status_code == 200
        assert patient_question.json()["status"] == "refused_patient_context"

        allowed = client.post(
            "/system/assistant",
            headers={"X-Demo-Role": "security_admin"},
            json={"question": "What is the model and governance status?"},
        )
        assert allowed.status_code == 200
        assert allowed.json()["status"] == "answered"
        answer = allowed.json()["answer"].lower()
        assert "model artifact" in answer
