...
class TestIndexedResolution:
    ...
    def test_live_full_mimic_resolve_is_bounded_not_full_index(self, resolver, monkeypatch):
        cr, proc = resolver
        _seed(proc, 300)
        cr._CASE_CACHE.clear()
        cr._PARTIAL_CASE_CACHE.clear()
        monkeypatch.setattr(cr, "_uses_live_credentialed_full_mimic", lambda ds: True)

        def fail_full_index(*args, **kwargs):
            raise AssertionError("live full-MIMIC resolve must not build the full index")

        monkeypatch.setattr(cr, "_get_resolved_cases", fail_full_index)
        uid = cr.pseudonymous_case_uid("MIMIC-IV-ED-Full-v2.2", 30000005)
        rc = cr.resolve(uid)
        assert rc is not None and rc.case_uid == uid
        assert cr.resolve("MIMIC-IV-ED-Full-v2.2~deadbeef") is None
        assert cr._CASE_CACHE == {}
        assert cr._PARTIAL_CASE_CACHE["mimic_full"]["upto"] >= cr.MAX_PAGE_SIZE
