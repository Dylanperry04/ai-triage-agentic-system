...
class TestSystemMessagesEnforceSafetyBoundary:
    ...
    def test_live_case_uid_prompts_focus_on_category_reasoning_not_vital_recap(self):
        prompts = "\n".join([
            CASE_UID_INTAKE_SYSTEM_MESSAGE,
            CASE_UID_VALIDATION_SYSTEM_MESSAGE,
            CASE_UID_SAFETY_REVIEW_SYSTEM_MESSAGE,
        ...
        assert "category_reasoning.primary_drivers" in prompts
        assert "category_reasoning.model_probability_summary" in prompts
        assert "do not list every vital sign" in prompts
        assert "do not re-list normal vitals" in prompts
        assert "why did the system show this exact acuity/category" in prompts
...
class TestRunTeamExplanation:
    ...
    def test_case_uid_team_missing_autogen_dependency_degrades_cleanly(self, monkeypatch):
        from app.agents.autogen_team import AutoGenDependencyError
        ...
        def _missing_dependency(_config):
            raise AutoGenDependencyError("autogen_ext missing")

        monkeypatch.setattr(mat, "build_model_client", _missing_dependency)
        result = asyncio.run(run_case_uid_team_explanation(case_uid, {"case_uid": case_uid}))

        assert result["status"] == "NOT_CONFIGURED"
        assert result["agent_turns"] == []
        assert "AutoGen packages are not installed" in result["final_explanation"]
        assert result["safety_failures"] == ["AUTOGEN_RUNTIME_NOT_INSTALLED"]
...
class _CloseFailsReplayClient(ReplayChatCompletionClient):
    """Replay client whose close() raises AFTER the run has succeeded, to prove
    a cleanup failure does not discard a valid multi-agent result."""

    closed_attempted = False

    async def close(self) -> None:  # type: ignore[override]
        type(self).closed_attempted = True
        raise RuntimeError("simulated transport close failure")


class TestCleanupFailureDoesNotDiscardResult:
    """Regression: a model_client.close() exception in the finally block must be
    logged and swallowed, never allowed to replace a successful agent result."""

    def test_successful_result_survives_close_failure(self, monkeypatch):
        script = _full_four_agent_script(stay_id=1)
        monkeypatch.setattr(
            mat, "load_azure_config",
            lambda: {"endpoint": "x", "api_key": "x", "deployment": "x", "api_version": "x", "model": "x"},
        )
        _CloseFailsReplayClient.closed_attempted = False
        monkeypatch.setattr(
            mat, "build_model_client",
            lambda config: _CloseFailsReplayClient(chat_completions=script, model_info=_MODEL_INFO),
        )
        # Must NOT raise, and must return the real PASS result.
        result = asyncio.run(run_team_explanation(1, cases_path=FIXTURES))
        assert _CloseFailsReplayClient.closed_attempted is True, "close() should have been attempted"
        assert result["status"] == "PASS"
        assert len(result["agent_turns"]) == 4
        assert result["safety_failures"] == []
