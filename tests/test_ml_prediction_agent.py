...
class FixedProbabilityMimicModel:
    def __init__(self):
        import numpy as np
    ...
    def predict_proba(self, X):
        import numpy as np
        return np.asarray([[0.20, 0.45, 0.25, 0.10, 0.00] for _ in range(len(X))])
...
class TestOnlyFullMimicIsAPredictionSource:
    ...
    def test_note_withholds_prediction_without_dataset_disclosure(self):
        r = run_ml_prediction(_make_input(source_dataset="Kaggle-KTAS"))
        assert "No live prediction model is available" in r.model_note
        assert "clinician review required" in r.model_note.lower()
        assert "MIMIC" not in r.model_note
...
class TestFullMimicModelRouterFailClosed:
    ...
    def test_v2_artifact_applies_validation_tuned_decision_rule(
        ...
        art = tmp_path / "mimic_v2.joblib"
        joblib.dump({
            "model": FixedProbabilityMimicModel(),
        ...
        assert r.high_acuity_research_estimate == pytest.approx(0.65)
