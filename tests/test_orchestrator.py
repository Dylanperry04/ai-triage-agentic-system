...
class TestLlmEvidencePackageTemperatureLabelling:
    ...
    def test_clinician_question_is_passed_to_llm(self, monkeypatch):
        captured = {}

        def fake_run_llm_explanation(case_evidence, clinician_question=None):
            captured["question"] = clinician_question
            from app.schemas.workflow import ExplanationResult
            return ExplanationResult(explanation_status="TEST_STUB")
        ...
        run_workflow(
            ...
            clinician_question="Why is the oxygen saturation concerning?",
        )

        assert captured["question"] == "Why is the oxygen saturation concerning?"
