...
class TestEvidenceMinimisation:
    def test_drops_unknown_fields(self):
        ev = {"chief_complaint": "FEVER", "ml_research_estimate": {"x": 1},
              "category_reasoning": {"primary_drivers": ["fever pathway"]},
        ...
        assert "category_reasoning" in m
...
class TestBoundaryAuthorisation:
    def test_nurse_denied(self, tmp_path, monkeypatch):
    ...
    def test_security_admin_itd_allowed(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ACCESS_AUDIT_DIR", str(tmp_path))
        ...
        assert d.minimised_evidence == {"chief_complaint": "X"}
