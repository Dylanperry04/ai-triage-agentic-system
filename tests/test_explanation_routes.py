...
class TestBuildVerifiedEndpointEvidence:
    def test_missing_vital_genuinely_surfaces_in_evidence(self, monkeypatch, tmp_path):
        """
        End-to-end check using a real isolated processed-cases file,
        confirming the fix holds through the full
        build_verified_endpoint_evidence() call path, not just the helper
        function in isolation.
        """
        import json
        from app.config import settings

        case = _make_case(stay_id=42, o2sat=None)
        ...
        evidence = build_verified_endpoint_evidence(42)
        assert evidence["missing_required_fields"] == ["o2sat"]
        assert "MISSING_CRITICAL_VITAL:o2sat" in evidence["data_quality_flags"]
