...
## K. Workflow-state and overdue-vitals operations
- [ ] **App** - Current workflow state reads fail closed if durable point reads
      fail; a database outage must not be interpreted as "no state".
- [ ] **App** - Escalation confirmation/resolution and case discharge are
      terminal transitions; repeated/conflicting terminal actions return
...
- [ ] **Both** - Verify the 210-minute vitals clock uses the latest persisted
      ...
      suppresses duplicate alerts until vitals change.
- [ ] **App/IT** - Workflow queue access is granted by
      `can_view_workflow_queue`, not generic case-view permission. Researchers
      must not receive operational clinical worklist access unless governance
      explicitly approves a new role.
