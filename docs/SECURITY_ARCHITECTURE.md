...
## 11. Update (v13→v18): enforcement architecture + pseudonymous identifiers

### 11.1 React UI + FastAPI backend, single enforcement path (v18)

> v18: the UI is now React, served by the FastAPI backend as static files on the
> same origin. The single-enforcement-path property below is unchanged — the
> backend remains the sole boundary; only the frontend technology changed. The
> retired Streamlit frontend is kept in-repo for reference.
